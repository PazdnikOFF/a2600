...
| C5 | **`KFactoryOptions` — КОЛЛИЗИЯ ИМЕНИ (UI-порт):** реф. `KFactoryOptions` — это Qt-ДИАЛОГ (KDialog-подкласс, Ui_KFactoryOptions::setupUi, 4 группы: авто-тест/восстановление/прочее/инструменты, ~25 виджетов), содержащий И UI, И логику test-env (ReadTestEnv/SaveTestEnv/CheckScope). НО у нас УЖЕ есть `app/src/sys/KFactoryOptions.h` — data-класс, занявший имя ПОЛОВИНОЙ-логикой (SetCheckScope/GetTestConfPath/ReadTestEnv/SaveTestEnv, self-test `factoryopt`). Порт диалога под тем же именем → redefinition. | Разметка диалога РЕВЕРСИРОВАНА 2026-07-21 (спека в истории субагента). НЕ портировать вслепую: решить коллизию как с KDocumentGenerator — либо СОВМЕСТИТЬ UI-диалог с существующей logic-половиной в один KFactoryOptions (реф. один класс), либо переименовать sys-класс. Пока — взят другой диалог. |
| C6 | **`KVideoCal` — КОЛЛИЗИЯ ИМЕНИ (UI-порт):** реф. `KVideoCal` — это Qt-ДИАЛОГ (KDialog, non-modal, SetKStyle(2), ctor @0x63b4e8, Ui_KVideoCal::setupUi @0x63ca98, resize 300×982, 6 групп: TR_CShape/COffset/CArea/EType/DSize + «图像参数»(AGC/AEC) + frame Cntrd/Exit; ~50 виджетов combo/spin, все stock Qt; заголовок группы 图像参数 хардкод-китайский). НО у нас УЖЕ есть `app/src/video/KVideoCal.h` — data-класс (off-device ядро: GetCenterOffset*Range/GetDefaultVideoCornerCutting/SaveDisplayArea→KDisplayOption), И его хедер ЯВНО откладывает UI-часть в «Фазу E/F». Порт диалога под тем же именем → redefinition. | Разметка ПОЛНОСТЬЮ РЕВЕРСИРОВАНА 2026-07-21 (спека в истории субагента ae4fd2ce). НЕ портировать вслепую: решить коллизию как с KDocumentGenerator — СОВМЕСТИТЬ UI-диалог с существующей logic-половиной в один KVideoCal (реф. один класс: ExitAdjustMode/Save*/InitWidget) ЛИБО переименовать. Пока — взят другой диалог. Пречек `class KVideoCal` в app/src поймал до написания кода. |
