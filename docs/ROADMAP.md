...
- ⬜ **KCamera** (38) — ОТКРЫТО, off-device: весь внешний вызов это KEncStyle/KAccount/
  LogPrintf/usleep. `InitCameraSeriesMap` — 7 записей. Дешёвое продолжение KEndoScope.
- ⬜ **KLcdProxy** (103) — ОТКРЫТО, off-device: это СЕМАНТИЧЕСКИЙ слой (состояние →
  QList<_KeyVlaue>) над транспортом, а не транспорт. Нужна заглушка KUiMsgProxy.
