...
**ОЧЕРЕДЬ ФАЗЫ F ПОСЛЕ ИСЧЕРПАНИЯ ВИДЖЕТ-СЛОЯ (инвентаризация 2026-07-23 по символам
`staticMetaObject`, см. PROGRESS §10):** непортированные QObject-классы —
KBackGround, KCalendarTextNavigator, KCalendarView, KDICOMSrvListItem,
...
бинарнике, vtable не адресуется, сигнал не испускается).
✅ **СЛОЙ НАВИГАЦИИ `Open*` — 22 из 25 СДЕЛАНЫ** (`ui/KUiNavigation.*`, self-test `opendlgs`).
Не портирована только `OpenProgressBar` (мёртвая ветка целиком); `OpenUpdateMng` сделан
вместе с классом-роутером `KUpdateMng`. Следующий срез UI после QObject-очереди — именно свободные функции.
...
выдуманные коды типов KSystemStatus и инвертированные дефолты KProjectSet.
⛔ **KCalendarView / KCalendarTextNavigator — НЕ ПОРТИРОВАТЬ**: это переименованные ПРИВАТНЫЕ
классы Qt5 из форкнутого вендором `qcalendarwidget.cpp` (набор методов совпадает с upstream
1:1). Наш `KCalendarWidget : QCalendarWidget` получает их из самой Qt.
...
✅ **KImgListCell — СДЕЛАН** (self-test `imglistcell`), заодно исправлены выдуманные дефолты
KImgList-прямоугольников на референсные 270×210 / 270×202.
✅ **KOsdSpinBase / KOsdSpin02 / KOsdReturnLabel — СДЕЛАНЫ** (self-test `osdspin02`), заодно
починена база KFrame (реальные поля + стили) и навигация KOsdSubMenu. ⭐ Установлено: класс
`KOsdSpin` в прошивке НЕ инстанцируется (мёртвый код) — живой спин это `KOsdSpin02`.
...
| C5 | ~~`KFactoryOptions` — коллизия имени~~ **РАЗРЕШЕНА 2026-07-23: СОВМЕЩЕНИЕМ** (как C6/KDocumentGenerator). Реф. — один класс с UI и логикой; наш sys-класс уже моделировал «состояние виджетов + логику», поэтому диалог надстроен над ним, а `checkScope_` остался единственным источником истины (реальный `check_scope` — его зеркало). self-test `factoryopt` не тронут. | Разметка сверена дизасмом `Ui_KFactoryOptions::setupUi` @0x62bfa8 + `retranslateUi` @0x62b1c8 + ctor @0x629de8 + `InitWidget` @0x628f78: KDialog, SetKStyle(2), resize 460×751, minimumSize(0,22), заголовок TR_FOptions; 4 QGroupBox (TR_AgTest/TR_Rcvry/TR_Otr/TR_Tls) + спейсер, ~30 виджетов, objectName'ы 1:1 (включая реф. опечатку `cmd_prototypemode` и `btn_debug`, который на деле QComboBox). Коды возврата 0x11..0x16 пишутся прямо в поле +0x58 + close(), НЕ через setResult/done. Превью-режим `factoryoptdlg`, скриншот сверен. |
| C6 | ~~`KVideoCal` — коллизия имени~~ **РАЗРЕШЕНА 2026-07-23: СОВМЕЩЕНИЕМ** (как с KDocumentGenerator). Реф. `KVideoCal` — один класс с UI И логикой; наш `app/src/video/KVideoCal.h` был набором СТАТИЧЕСКИХ методов (GetCenterOffset*Range/GetDefaultVideoCornerCutting/SaveDisplayArea), поэтому диалог надстроен прямо над ними без ломки self-test `videocal`. | `KVideoCal : KDialog`, non-modal, SetKStyle(2), resize 300×982; разметка сверена дизасмом `Ui_KVideoCal::setupUi` @0x63ca98 + `retranslateUi` @0x63bc50: 6 QGroupBox (TR_CShape/TR_COffset/TR_CArea/TR_EType/TR_DSize/«图像参数») + нижний QFrame (TR_Cntrd/TR_Ext), ~50 стоковых виджетов, objectName'ы 1:1. Превью-режим `videocaldlg`, скриншот сверен. |
| C7 | **`GetReadOnlyBaseDir()` без хвостового слэша — ГРАБЛИ ПУТЕЙ:** `KEnvConfig::GetInstance().GetReadOnlyBaseDir()` (=`KSystem::ProjectPresetPath()`, через `QDir::absoluteFilePath`) возвращает путь БЕЗ завершающего `/` (`…/system/presetdata/syspreset`). Строковая конкатенация `base + "mainapp/application/…"` даёт склейку `sysprysetmainapp` → битый путь → `QPixmap::load` возвращает null, виджет невидим. Существующие self-test'ы это НЕ ловят: они сравнивают строки (обе стороны склеивают без слэша одинаково), файлы не грузят. | Строить путь через `QDir(base).absoluteFilePath("mainapp/…")`, НЕ строковым `+`. Поймано при сверке скриншота KPagePushButton (пейджер-иконки не грузились). Тот же риск у любого нового кода, грузящего ассеты от `GetReadOnlyBaseDir()`/`GetUsrDir()`. |
