...
- **Реализовано классов (по имени):** ~50 (эта сессия +10: KVideoCal, KUpdateManifest,
  KSysReportTempletCfg, KReportDBTableHandler-пагинация, KSaveFile, KUserOsdSet, KEntityService, KReportDisplayParam, KEndoInfoServerConfig, KRemoteSwitchConfig) +
...
- **Проверяемое off-device ядро (31 self-test-режим, все PASS):** plreg, filt, dicom, report,
  ...
  savefile, osdset, dbservice, dispparam, endoinfo, remoteswitch.
...
2. ✅ ЧАСТИЧНО **KVideoSet** (64) — СДЕЛАНО. Оркестрация Set*Level (ColEnh/ImgEnh/Denoise/
   ...
   Self-test `videoset`. ✅ KUserOsdSet (OSD-меню) СДЕЛАН отдельно (см. ниже). ОСТАЛОСЬ:
   corner-cut (device-геометрия).
...
2. **DICOM (БД off-device, сеть device):** ✅ ЧАСТИЧНО — KEntityDicom расширен tb_DcmStudy/tb_DcmSeries
   ...
   device), генерация Secondary Capture .dcm из JPEG+данные (device-DCMTK).
...
- ОСТАЛОСЬ (device/UI, Фаза E/F): KUpdateMng/KUpdatePrepare/KProgressTask/KFactoryOptions —
  распаковка rar (ExecRarCmd/StartDecompress), применение патча/прошивка (ExecUpdCmd,
  CalcFileMd/CheckUpdateFileMd — MD5 файлов пакета), реверс формата lcd_upd/образов (FAT),
  Qt-оркестрация с прогресс-баром.
