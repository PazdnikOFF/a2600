...
9. ✅ **KSmallLangTranslate** (22) — СДЕЛАНО (off-device целиком). Раскладки «малых
   ...
   Self-test `smalllang`.
   ОСТАЛОСЬ: расшифровать `g_strKeysym_S40`/`g_strKeysym_S50` (.bss, 233/357 строк) —
   это даст авторитетную таблицу «код клавиши ↔ имя» и наполнит E_SONO_KEY именами;
   Qt-обёртка KSmallLangInputMethod — отдельная UI-задача.
