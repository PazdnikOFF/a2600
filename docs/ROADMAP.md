...
- **Реализовано классов (по имени):** ~41 (+KVideoCal off-device ядро) + namespace `KSystem`.
  Предыдущая сессия +15:
...
- **Проверяемое off-device ядро (21 self-test-режим, все PASS):** plreg, filt, dicom, report,
  account, thesaurus, userset, coldlight, version, project, statistic, sysstatus, quickinput, style,
  examcfg, exam, filebackup, videoset, dsreal, dsdemo, videocal.
