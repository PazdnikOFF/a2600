...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 61  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **61 self-test-режим** (все PASS, регрессия —
...
затронуто 69 классов / 714 методов (11.1%)**. Это нижняя оценка (считает совпадение имён;
...
**ПОСЛЕДНЕЕ (эта сессия): `KEncSettings`** (`app/src/sys/`, self-test `encset`) — аксессор
