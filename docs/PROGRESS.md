...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 65  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **65 self-test-режимов** (все PASS, регрессия —
...
затронуто 73 класса / 724 метода (11.3%)**. Это нижняя оценка (считает совпадение имён;
...
**ПОСЛЕДНЕЕ (эта сессия): `KTitleTableBlock`** (`app/src/report/`, self-test `titletableblock`) —
