...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 78  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **78 self-test-режимов** (все PASS, регрессия —
...
**ПОСЛЕДНЕЕ (эта сессия): `KThirdPartyLegalNotices::ReadLegalNoticeText`** (self-test
...
templetmodel, dimmer, printdata, legalnotice.
