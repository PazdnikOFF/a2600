...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 71  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **71 self-test-режим** (все PASS, регрессия —
...
затронуто 80 классов / 798 методов (12.4%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KReportTemplateManager` — ФЕЙТФУЛ-API (self-test `reporttmplmgr`)** —
