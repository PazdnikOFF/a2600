...
ENDO_ROOT=$ER ui_preview opendlgs                     # self-test слоя навигации Open* (гейты по роли/QR/контролю, центрирование, карта переходов)
ENDO_ROOT=$ER ui_preview poweroff                     # self-test экрана выключения + моста прогресса + сортировки DICOM-списка
ENDO_ROOT=$ER ui_preview recorditem                   # self-test ячейки записи OSD (KRecordItem) + референсные коды типов KSystemStatus
ENDO_ROOT=$ER ui_preview osdroot out.png              # РЕНДЕР реального корневого OSD-меню (KOsdMenu::InitWidget со всеми ячейками)
ENDO_ROOT=$ER ui_preview evtfilters                   # self-test шести малых UI-классов (KEventFilter/KGlobalEventFilter/KLabelHoverEventFilter/KTableWidget/KPosNameLineEditDelegate/KBackGround)
ENDO_ROOT=$ER ui_preview patnavitem                   # self-test строки лево-навигации (KPatientListWidgetItem + setElidedFrame + KHoverEventFilter)
ENDO_ROOT=$ER ui_preview imglistcell                  # self-test ячейки превью снимка (KImgListCell + 4 ячейки в KImgList)
ENDO_ROOT=$ER ui_preview osdspin02                    # self-test живого OSD-спина (KOsdSpinBase/KOsdSpin02/KOsdReturnLabel + стили базы KFrame)
ENDO_ROOT=$ER ui_preview flexbtn                      # self-test гайда кнопок пульта ДУ (KFlexEndoBtnGuide: геометрия строк + таблица KUserSet::GetFunctionName)
ENDO_ROOT=$ER ui_preview flexguide out.png            # РЕНДЕР гайда кнопок гибкого эндоскопа (KFlexEndoBtnGuide, канва 186×136)
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **128 self-test-режимов** (все PASS, регрессия —
`tools/selftest.sh`; последний прогон — в контейнере на hermes, PASS 128 / FAIL 0).

**🔬 РАЗБОР АПДЕЙТЕРОВ ПРОШИВКИ + ПРОТОКОЛ МК-ПЛАТЫ (2026-07-24).** Дизасм на hermes
`update/cmd/{hmiupdate,lcdupdate,xc7a200tupdate,xc7a75tupdate}` (aarch64 ELF с debug_info,
исходники *.c/*.cpp) — ground truth device-протоколов, НЕДОСТУПНЫЙ из X2000. Self-test
`mcuuart`, модуль `ctrl/KMcuUart` (чистые примитивы; транспорт — device за швом).
- ⭐ **МК-плата на `/dev/ttyPS1`, 115200 8N1** (hmiupdate/lcdupdate InitUartDev: SetCommSpeed
  0x1002=B115200, SetCommAttr 8/0/1). Checksum — `crc_8` @0x4017ac: **рефлективный CRC-8,
  полином 0xB8, init 0** (тот же в hmiupdate и lcdupdate).
- **Кадр управляющего канала** (lcdupdate SendOneMsgToUart @0x25b0, сверено дизасмом):
  `AA | cmd(BE16) | len | payload | crc8check | xor`, где `crc8check` = **комплемент**
  crc_8 (`~crc & 0xFF`, пусто→0xFF, CrcCheck @0x21e0), `xor` = XOR{len,cmdHi,cmdLo,payload,crc}.
  Команды: **start 0x0444 → ack 0x8444, busy 0x0445 (стирание флеша), data 0x8445 (SPI,
  256-байт пакеты), end 0x0446/0x8446**.
- ⛔ **FPGA-апдейтеры — БЕЗ МК-протокола** (прямое программирование флеша, device): `xc7a200t`
  (Artix-7 200T) — SPI `/dev/spidev0.2` + GPIO CS, JEDEC-команды флеша (`cmdDatas`);
  `xc7a75t` (75T) — MTD `/dev/mtd0` (ioctl MEMERASE + write по CLI-offset). `lcdupdate` —
  гибрид: UART-контроль + данные по SPI `/dev/spidev0.1`.
- **Механизм обновления** (шелл `update/cmd/*_update` + бинарники): `update.ini` (порядок,
  Shell/UpdateTime/IsNeedUpdate/Param по компоненту), `history/order.ini` (Type0..N:
  app→hmi→pap→papp00..80→lcd→cleanup), `history/versiontree.ini` (версии `56.XX.YY.ZZ.AA.BB`),
  `history/md5tree.ini` (MD5). Читатель — реф. `KUpdateConf` (order/versiontree/md5tree +
  MD5-проверка); у нас off-device ядро уже в `KUpdateManifest` (self-test `update`).
- ⚠️ КОЛЛИЗИЯ (учтено): не дублировать `KUpdateManifest` — оркестратор обновления off-device
  уже там; `KUpdateConf` у нас = только matchedversion.ini. Проверять `class <Name>` ДО правок.

**🔬 ОБРАЗЫ ПРОШИВОК ПЕРИФЕРИИ ИДЕНТИФИЦИРОВАНЫ (2026-07-24, «разбери всё»).** `update/history/
*.bin` — прошивки периферийных плат, все device (нашего кода не дают, интерфейс = `KMcuUart`):
- `hmi_upd_*.bin` (230 КБ) — прошивка МК-платы: векторная таблица Cortex-M (SP=0x10007948,
  обработчики в 0x1a04xxxx), маркеры `5a5a5a5a`. Внутренности МК, не наш стек.
- `pap/papp00..80_upd_*.bin` (3.8–9.7 МБ) — **битстримы Xilinx 7-series FPGA** (сигнатура
  синхры `aa99 5566` на 0x30). Битовая конфигурация PL под разные типы эндоскопов, не код.
- `lcd_upd_*.bin` (8 МБ) — **FAT12-образ** UI-ассетов 8″ панели: ~78 .BIN (кастомный битмап
  `05 <w> <h> <flags>` + RGB-прогоны) — иконки/экраны кнопок и состояний панели (LAMP, LOCK,
  BALANCE, CONTR1-3, MOIRE, FROZEN, TRANS, стрелки, USB, VIDEO, LOGO1-6, ON/OFF, PARAM, SYS…).
  Дисплейные данные САМОЙ панели (рисует её MCU) — device; документируют словарь UI панели (§8).
- ⛔ ИТОГ: весь дамп прошивки разобран. Портируемого off-device больше НЕТ — остальное это
  device-внутренности периферийных плат (МК/FPGA/LCD), с которыми наш стек ОБЩАЕТСЯ (протокол
  `KMcuUart`), а не реимплементирует. Рантайм-командный словарь МК (`HmiMcu`→`KMsgbuffer`→
  comm-поток→UART) реверсится, но его семантика проверяема только на приборе (не выдумываем).

**🎯 ВЕХА: РЕАЛЬНОЕ ПРИЛОЖЕНИЕ `endostation` СОБИРАЕТСЯ, ЛИНКУЕТСЯ И СТАРТУЕТ (2026-07-24).**
До сих пор проверялся только харнесс `ui_preview`; настоящая цель `endostation` (main.cpp →
KMainCtrlThread::Init) никогда не линковалась. Первая сборка на hermes с GStreamer 1.22
(образ `a2600-qt5gst:bookworm`, см. [[a2600-hermes-linux-build]]):
- ⭐ **ВСЕ .cpp скомпилировались целиком** (HAVE_GST-ветки KVideoProxy и пр., которые
  ui_preview не трогает) — падала ТОЛЬКО линковка, и лишь из-за пропуска `sqlite3` в
  `target_link_libraries(endostation)` (реальный баг конфигурации, фикс `d0ccdf5`).
- После фикса: `endostation` линкуется (84 МБ debug), **стартует** offscreen, HAL штатно
  уходит в заглушку («HAL stub — периферия недоступна»), грузит тему/PNG-ассеты прошивки,
...
- Синглтон — свободная `GetKAutoTestThread()` @0x6fc830 (ленивое `new` 0x128, delete нет).
  Поля: **+0x10** флаг остановки, **+0x14** FileExecCount, **+0x18** ExecIndex,
  **+0x124** флаг проверки логов. `ResetFileExecCount` @0x6fb4c8 — ОДНА невыравненная
  8-байтовая запись: {+0x14, +0x18} = {1, 0}. ctor подписывается на два IPC-канала
  `StartProcMsgManager(0x85000000)` и `(0x86000000)` (device).
- `start(Priority)` @0x6fc230 **переопределяет невиртуальный** `QThread::start`: если поток
  уже бежит — молча выход; иначе снять флаг, `Setup_uinput_device()`, `QThread::start`.
  `stop` @0x6fc290 — только флаг + `Remove_uinput_device()` (без `wait()`).
- `run` @0x6fc618: цикл до флага, **usleep 100 мс** (0x186A0), затем разбор очереди пар
  {key, event} (в реф. — глобальный `std::deque`): пара {0,0} пропускается,
  **event == 0 → KeyboardSimulation(key), иначе → PanelKeySimulation(key, event)**.
- ⭐ `KeyboardSimulation` @0x6fc558 — точная раскладка: клавиша = младшие **28** бит
  (дальше `QtKey2InputKey`, device), модификаторы — биты **29 Shift / 30 Ctrl / 31 Alt**
  (те же, что кладёт скрипт-парсер в `KEY_CONFIG`). Дизасм сдвигает значение на 4 вправо
  и проверяет биты 25..27; порядок ветвления — от полной комбинации к пустой:
  CtrlAltShift → CtrlAlt → CtrlShift → AltShift → Ctrl → Alt → Shift → Key.
- `PanelKeySimulation` @0x6fc2a0: сообщение **0x41A**, раскладка +8 = key (u16),
  +10 = (value >> 16) & 0xff, +11 = value & 0xff, затем `KSrvBaseThread::PostMsg(1, &msg)`.
- `GetLogPath` @0x6fc348 = `KSystem::LogPath()` + `APPlog%04d-%02d.txt` по ТЕКУЩИМ году и
  месяцу (при отказе `localtime_r` реф. подставляет 1900-01). Добавлен
  **`KSystem::LogPath()` @0x672340 = AppPath() + "logfile/"**.
- `IsAutoTestStart` @0x6fc818 — ⚠️ НЕ «!= 0»: `tst w0, #0xfffffffd` ⇒ ложь ровно при
  **0 и 2**, истина при 1, 3, 4… Добавлен геттер `KSystemStatus::AutoTestStatus()`.
...
- `KSelfTest` @0x715240 (KDialog, sizeof 0x60) — НЕ интерактивен: весь отчёт собирается
  ПРЯМО В КОНСТРУКТОРЕ (`startCheck()` → три проверки → `displayInfo()`), кнопка только
  закрывает окно. Вёрстка: QGridLayout(`gridLayout`, vSpacing 20,
  margins(-1,50,-1,20)); QListWidget(`listWidget`) на (0,0,1,3); QPushButton(`btn_OK`,
  min 75×0, max 100×16777215) на (1,1,1,1); `SetKStyle(2)` (ширина 460);
  титул окна `TR_Dlg`, титул-бар `TR_STest`.
- `startCheck` @0x7150f8: кладёт `TR_INPTSTest:`, зовёт **checkProcessor → checkEndo →
  checkLight** (именно в этом порядке), кладёт `TR_ICompleted`, в конце
  `removeDuplicates()`. Каждая проверка добавляет строку ТОЛЬКО при проблеме.
- `checkProcessor` @0x714d00: пустой `KSystemSet::GetProcessorSN()` → `TR_IPSNNI`;
  `<DataPath>/protected/syspreset/testenv.ini`, ключ **AgeTest/IsAgeTest** (дефолт false)
  → при false строка `TR_IPATestNE`. (Файла нет в дампе прошивки — на стенде создаётся.)
- `checkEndo` @0x714690 — читает `GetEndoScope()->GetEndoInfo()/GetEepromData()`
  (у нас акцессоры называются `EndoInfo()/EepromData()`), поля уже разобраны в
  `_EndoInfoStruct`/`_EepromInfo`:
  • `sModel` пуст → `TR_EModelNC`;
  • `sEndoID` ИЛИ `sWarrantyDate` пусты → `TR_EInfomationNI` (sic, «Infomation»);
  • ⚠️ **ПОЛЯРНОСТЬ ОБРАТНАЯ:** `usedCount != 0 || !sServerContract.isEmpty() ||
    !sComment.isEmpty()` → `TR_IROEndoscopeNE`, т.е. строка про эндоскоп, БЫВШИЙ В РАБОТЕ
    (легко прочитать наоборот — в дизасме это `cbnz`/`cbz` вперемешку);
  • `fixFlags`: бит 1 снят → `TR_WBMemoryNE`; `(fixFlags & 3) == 2` → `TR_VACalibrationNE`.
- `checkLight` @0x714c38: `used = GetLampUsedTime()`, `total = GetLampTotalUsedTime()`;
  `cmp #29 / ccmp #29 / b.ls` ⇒ строка `TR_CLATestNE`, если **хотя бы один <= 29**
  (порог 29, НЕ 30). Единственный device-вход класса → у нас инъектируемый провайдер
  `KSelfTest::SetLampTimeProvider` (дефолт {0,0}).
- `displayInfo` @0x7145b8: `clear()`, затем `insertItems(count(), m_infoList)` — ⚠️ позиция
  считается ПОСЛЕ очистки, т.е. всегда 0 (безобидный квирк реф.).
  `timerToUpdate` @0x714600 — тело ровно `b displayInfo`, полный синоним; подписчиков нет.
- `OpenKSelfTestDlg()` @0x7157c8 — 23-я портированная свободная `Open*`: LogPrintf,
  `new` на куче, `exec()`, виртуальное удаление. Осталась непортированной только
  `OpenProgressBar` (мёртвая ветка).

**✅ KTimeMng + «ЗАЩИЩЁННЫЙ» system.ini (лицензия и продуктовая идентичность), 2026-07-23.**
Self-test `timemng`; модуль `sys/KTimeMng.{h,cpp}`, дополнены `KSystemSet`, `KProjectSet`,
`KUiMsgProxy`, `KSystemStatus`.
- ⭐ **ПРИЁМ ИНВЕНТАРИЗАЦИИ ИСПРАВЛЕН:** символ `staticMetaObject` лежит в ДИНАМИЧЕСКОЙ
  таблице → нужен `nm -D`, а наши `tools/symmaps/*.symmap` его не содержат вовсе
  (`tools/rev.sh sym staticMetaObject` даёт пусто и создаёт ложное впечатление «всё
  портировано»). Рабочая команда — на hermes:
  `nm -D ~/a2600ref/bin/X2000 | grep -o '_ZN[A-Za-z_0-9]*16staticMetaObjectE'`, дальше
  срезать `_ZN<N>` и `16staticMetaObjectE`. Итог: из 259 Q_OBJECT-классов непортированы
  пять — KSelfTest, KAutoTestThread, ColdLight, HmiMcu, KDICOMSrvListItem (последний
  заменён у нас структурой `DicomSrvRow`); ColdLight/HmiMcu — device (фаза E).
- `KTimeMng` @0x458378 (QObject, sizeof 0x30) — синглтон БЕЗ `Instance()`: доступ через
  свободную `GetKTimeMng()` @0x458800 (ленивое `new`, delete никогда). ctor: поле +0x10 =
  `GetKUiMsgProxy()`, остальные три — nullptr, затем `InitTimer()`.
- `InitTimer` @0x458160 заводит **три** QTimer'а, все с `parent=nullptr` (владения нет):
  **1000 мс → `Timedisplay`** (+0x18), **60000 мс → `EachDayMC`** (+0x20),
  **1500 мс → `InitMC`** (+0x28). При провале `new (nothrow)` реф. пишет в лог
  `"%s : out of memory"` и продолжает — поле остаётся nullptr.
- `Timedisplay`: `UpdateSystemtime(QDateTime::currentDateTime().toString(<формат даты>
  + "\nhh:mm:ss"))`. ⚠️ Разделитель — **перевод строки**: дата и время выводятся в две
  строки. Формат даты в реф. берётся из `GetKSystemSetStatus()` (= `GetKSystemSet()->
  CurrentStatus()`, поле +0x0; +0x8 там — язык), у нас — `KSystemSet::DateFormat()`.
- `EachDayMC`: таймер минутный, но тело работает **только в 00:00** (`hour()!=0` или
  `minute()!=0` → выход). В полночь: `CheckMachineControl()`, затем если
  `GetRemainDays().toInt() == 1` → `SetProductAuthFlag(0)` + сигнал
  `KSystemStatus::AuthMachineChange()` @0x8285f8. ⚠️ Квирк: в реф. `GetRemainDays().toInt()`
  вычисляется ДВАЖДЫ, первый результат отбрасывается (у нас один вызов — побочных
  эффектов нет).
- `InitMC`: одноразовый отложенный старт — `CheckMachineControl()` и `stop()` своего же
  таймера (+0x28).
- `StartRecTimer`/`StopRecTimer` @0x458100/@0x458148 — секундомер записи тикает от ТОГО ЖЕ
  1-секундного таймера: `connect(timer1, SIGNAL(timeout()), proxy, SIGNAL(UpdateRecTime()))`
  (сигнал→сигнал) и симметричный `disconnect`. Четвёртого таймера нет.
- В `KUiMsgProxy` добавлены три НАСТОЯЩИХ Qt-сигнала (реф. @0x82d840/@0x82d868/@0x82da90):
  `UpdateSystemtime(QString)` (⚠️ по значению), `UpdateRecTime()`, `CheckMachineControl()` —
  заглушкой-методом не обойтись, т.к. подключение идёт по строковому SIGNAL/SLOT.
- ⚠️ **«Защищённый» `system.ini` — ДРУГОЙ ФАЙЛ**, не тот, что читают обычные геттеры
  `KSystemSet`: `GetProtectedSysIniPath` @0x6582d0 = `KSystem::ProtectedPath()` +
  `"system.ini"` (каталог создаётся `mkpath`, если его нет) →
  `/home/root/data/protected/system.ini`. Доступ — `ReadProtectedValue` @0x658e80 /
  `WriteProtectedValue` @0x658da8 (обе открывают QSettings на каждый вызов).
- Реализована вся группа (дефолт у всех — ПУСТАЯ строка): `GetProductCN`,
  `GetLastValidDate`, `GetRemainDays`, `GetProductAuthFlag`, `GetAuthBinMD5` + сеттеры.
  ⚠️ **Ключи с опечатками вендора, оставлены как есть:** `System/ProductLastVaildDate`
  (Vaild), `System/ProductAuthbinMD5` (Authbin), `Product/ProductSerise` (Serise).
  ⚠️ `RemainDays` в реф. — **строка** (`Set/GetRemainDays(QString)`), вызывающие сами
  делают `toInt()`; `ProductAuthFlag` пишется `int`, а читается `QString` (асимметрия реф.).
- Продуктовая идентичность (тот же файл, но секция `[Product]`) с ВАЛИДАЦИЕЙ по спискам
  из project.ini: `GetProductSeries` @0x660610 (ключ `Product/ProductSerise`, список —
  `KProjectSet::GetProductSeriesList`), `GetProductModel` @0x660a28 (`Product/ProductModel`,
  список — `GetProductModelList(GetProductSeries())`, причём `GetProductSeries()` зовётся
  БЕЗУСЛОВНО), `GetProductRelaseVersion` @0x660ea0 (sic; `Product/ReleaseVersion`, дефолт
  **"V2.0"**, список — новый `KProjectSet::GetReleaseVersionList` @0x656128 =
  `[Option]ReleaseVersion` того же project.ini, прочитанный как `toStringList()`).
  ⚠️ **Если сохранённого значения нет в списке — берётся ПЕРВЫЙ элемент** (`array+begin*8`,
  т.е. `list.first()`, а НЕ last — операнд легко прочитать наоборот).
- ⚠️ `KProjectSet::IsFirstRegisterVersion` @0x655380 — **заглушка прошивки**: тело ровно
  `mov w0,#0; ret`, всегда false ⇒ ветка «версия = V1.0» в `GetProductRelaseVersion`
  мёртвая. Оставлена в коде ради читаемости.

**✅ АУДИТ ПОДСТАНОВОК stock-Qt В ПОРТИРОВАННЫХ ДИАЛОГАХ (2026-07-23).**
⭐ **ПРИЁМ:** для каждого нашего кастом-виджета найти в дизасме адреса его ctor, собрать
вызывающих (`bl <ctor>` внутри `Ui_*::setupUi`) и сверить с тем, что стоит в нашем
одноимённом диалоге. 18 кастом-виджетов × их пользователи = 34 пары «виджет↔диалог»;
расхождений нашлось **всего два** (остальное уже было портировано верно):
- `KReportEditUi`: `gridWidget_base_info` — это **KGridWidget** (реф. @0x4d83ec, следом
  `setFocusPolicy(NoFocus)` и свой QGridLayout), а у нас стоял голый QWidget.
- `KPatientListAddDlg`: поле даты рождения — **KPatientDateEdit** (плейсхолдер вместо
  пустой даты), а у нас — голый QDateEdit.
Оба заменены, скриншоты `reporteditui`/`patientadd` сверены — раскладка не поехала.
...
⭐ **ПРИЁМ ИНВЕНТАРИЗАЦИИ:** после закрытия QObject-очереди следующий срез UI —
СВОБОДНЫЕ функции: `nm | grep -oE '_Z[0-9]+Open[A-Za-z0-9_]+'` → имя = ровно N символов
после `_Z<N>`. Всего 46 `Open*`, не портированы были 25.
- Общая схема всех: `new` на куче → (иногда) центрирование → `exec()`/`DoModal()` → чтение
  результата → `delete`. Утечек в реф. НЕТ ни в одной.
- ⚠️ **Аргумент `QWidget*` у большинства — НЕ Qt-родитель**: диалог строится с parent=nullptr,
  а аргумент нужен ТОЛЬКО для центрирования `move((p.w-d.w)/2, (p.h-d.h)/2)` (деление
  целочисленное). Настоящим родителем аргумент становится лишь в `OpenUpdateAction` и
  `OpenUpdatePrepare`.
- **Гейты (точные условия):** `OpenControlInfoDlg` — хотя бы один из ЧЕТЫРЁХ режимов контроля
  (`IsStartTimeMc || IsStartEndoMc || IsStartEndoUseTimeCtrl || IsStartMatchProcessorCtrl`,
  все device → у нас инъектируемый провайдер, по умолчанию выкл.);
  `OpenDeviceInfoDlg` — `if (IsHideQRCode()) return;`; роль > 1 у `OpenLogViewDlg`,
  `OpenProcessorControlDlg`, `OpenUpdateMng`, `OpenUserSrvSetDlg`; роль > **3** у
  `OpenRecordCase` (иначе false).
- ⚠️ `KProjectSet::IsHideQRCode` @0x655600 — НЕ флаг конфига, а сравнение текущего стиля
  бренда с литералом **`"PyCkeun"`** (@0x83f540): у российского бренда QR-код скрыт.
- ⚠️ **`OpenUserSrvSetDlg` — не «открыватель», а ДИСПЕТЧЕР**: при роли <= 1 вместо сервисного
  меню открывает `OpenDeviceInfoDlg()` (tail-call), а после закрытия читает
  `KUserSrvSet::m_eJumpModule` (+0x68, ставит `InterfaceJump`) и открывает следующий экран:
  **6** ScopeInfo, **9** LogView, **10** ProcessorControl, **11** EndoControl, **13** UpdateMng,
  **16** ColdlightAdjust, **17** VideoCal.
- `OpenThesaurusSaveDlg` @0x4e3f40 — единственная, кто открывает через `KDialog::DoModal()`
  (@0x68b768 — ровно `b QDialog::exec`), остальные зовут `exec()` напрямую.
- Возвращают значение: `OpenRecordCase` (bool, поле +0x58), `OpenUpdateMng`/`OpenUpdatePrepare`
  (int, поле +0x58), `OpenProcessorSNSetDlg` (QString — код exec ИГНОРИРУЕТСЯ, отдаётся GetSetSN()).
- ⚠️ **Мёртвые точки входа:** `OpenProcessorSNSetDlg` @0x6fff40 и `OpenProgressBar` @0x60aae8
  (0 ссылок в бинарнике; вместе с `OpenProgressBar` мертва вся ветка `ExecProgressBar` →
...
Self-test `poweroff`.
- `KPoweroff` @0x626620 (QDialog, sizeof 0x40; вход — свободная `SystemPoweroff()` @0x626d18):
  grid с полями (-1,-1,-1,100), логотип 580×120 (Alignment 0x84), бар 580×30, метка 580×60
  с текстом `TR_SDown.` (⚠️ с точкой), окно Dialog|Frameless|StaysOnTop.
  ⚠️ **Прогресс-бар НЕ показывает реальный прогресс**: `maximum = 20`, таймер **50 мс**,
  значение = номер тика. На 20-м тике (ровно 1 с) уходит `SendToMainCtrl(0)`, дальше диалог
  ЖДЁТ завершения процессов `X2000Video`/`X2000Simulator`/`X2000` (`CheckProcStatus` @0x626300 —
  скан /proc, device-seam) и только тогда закрывается.
- `KToProgressDlgMsgDispatcher` — синглтон БЕЗ `Instance()`: доступ через свободную
  `ToDlgMsgDispatcher()` @0x4511f8 (Meyers-статик). ctor не делает ничего, кроме
  `qRegisterMetaType<KMessage>()`. Ровно **восемь** сигналов (Title+TotalProgress / Hide /
  ShowResultMsgBox / TotalProgress / SubProgress / TotalLabel / SubLabel /
  OneExamRecordUpdateFinish(KMessage)); все `Emit*` — чистые эмиттеры (три — хвостовые
  переходы прямо на сигнал). Эмитят `KDicomEventDeal`/`KDataOprEventDeal` (device),
  единственный потребитель — `KProgressDlg::InitConnect` @0x451320, где подключены все восемь;
  у нас слоты `On*` реализованы и подключены.
- `KDICOMSrvListModel::SortDicomItem` @0x5b8d60 портирован: два бакета —
  сначала `type != 1 && enabled`, затем остальные; внутри каждого — по времени добавления
  УБЫВАНИЮ (реф. `KDICOMSrvListItem::operator>` @0x6dd128 сравнивает ТОЛЬКО `QDateTime`).
  В `DicomSrvRow` добавлено поле `addTime` (реф. поле +0x18, ini-ключ `AddTime`,
  формат `yyyy-MM-dd hh:mm:ss`).

**✅ KRecordItem + ⚠️⚠️ ИСПРАВЛЕНЫ КОДЫ ТИПОВ KSystemStatus И ДЕФОЛТЫ KProjectSet (2026-07-23).**
Self-test `recorditem` (+ обновлён `sysstatus`), рендер — новый экран `osdroot`.
- ⚠️⚠️ **КОДЫ `SystemStatusChange` У НАС БЫЛИ ВЫДУМАНЫ** (перечисление шло подряд 0,1,2…).
  Реальные — константа `mov w1,#N` перед хвостовым переходом на
  `KSystemStatus::SystemStatusChange` @0x8284a8 в каждом сеттере (@0x621fb0..0x622430):
  **0 ViewType, 1 Video, 1 FullScreenUI (ТОТ ЖЕ код!), 2 EndoDisconnectImage, 3 Network,
  4 Freeze, 5 Agc, 6 CHb, 7 Record, 8 Lamp, 9 VlsMode, 10 LowLight, 11 DimmingType,
  12 LightLevel, 13 Brightness, 14 IsVideoCal, 15 MainScreenState, 16 StopWatch, 17 AirPump.**
  Было бы «запись = 2» вместо 7 — подписчик на приборе не сработал бы вообще.
- ⚠️ Семь сеттеров в реф. **НЕ оповещают** (тело — только запись поля, 8 байт кода):
  `SetIrisValue` @0x622260, `SetVideoPlayStatus` @0x622420, `SetPanelType` @0x621fb0,
  `SetWindowID` @0x621fb8, `SetAutoTestStatus` @0x6223a0, `SetRatioValue`, `SetIsEUDEndoType`.
  У нас они слали выдуманные типы — теперь молчат (self-test `sysstatus` пересчитан: 4 события).
- ⚠️ **Дефолты `KProjectSet` были инвертированы**: реф. `IsZoomEnable` @0x656970,
  `IsChbEnable` @0x656b10, `IsVideoRecordEnable` @0x656cb0 читают ini с `QVariant(true)`
  (`mov w1,#1` перед `QVariant(bool)`), у нас стояло `false`. Из-за этого запись считалась
  выключенной и `KRecordItem` не попадал в корневое меню. В поставке 12 product.ini с
  `RECORD=false` и 7 с `RECORD=true` — но дефолт при ОТСУТСТВИИ ключа именно true.
- `KRecordItem` @0x486160 (подкласс KOsdMenuCellGreyedWhenCameraDisconnected, своих полей нет)
  — единственная ячейка, у которой ИКОНКИ И ЗАГОЛОВОК меняются на лету:
  status **0** → `video_normal/video_select/video_select_grey.png` + `TR_Rcd`;
  status **2** → `recording_normal/recording_select/recording_select.png` (⚠️ серая иконка
  ТА ЖЕ, что выбранная) + `TR_RStop`; прочие значения игнорируются целиком.
  `SystemStatusChangeAct` фильтрует `type != 7`; `UsbStatusChangeAct` аргумент ИГНОРИРУЕТ;
  `CheckGreyedCondition` = камера не готова ИЛИ USB отключён; `HasSubMenu` → false.
  `ConfirmAct` @0x486290 — цепочка гейтов, финал: `LocatedMenu()->done(2)` (код «идём писать»).
  В `KOsdMenu::InitWidget` ячейка стоит ВТОРОЙ, за гейтом `IsVideoRecordEnable()`.

**✅ ШЕСТЬ МАЛЫХ UI-КЛАССОВ ОДНОЙ ПАЧКОЙ (2026-07-23).** Self-test `evtfilters`.
- `KEventFilter` @0x67a960 (QObject, sizeof 0x10, БЕЗ полей): ctor САМ ставит себя на qApp;
  `CreateInstance` @0x67a9e0 — function-local static, зовётся из `KAppThread::start` @0x626098.
  `eventFilter` @0x67aa78: на типах **2..7** (`sub w0,#2; cmp w0,#5`) дёргает
  `KForceLogout::RefreshActiveTime` (seam → сигнал `userActivity`); горячая клавиша скриншота —
  ⚠️ **ТОЧНОЕ** равенство `modifiers() == Qt::ControlModifier` (не проверка бита ⇒ Ctrl+Shift+P
  НЕ срабатывает) и `key()` == **0x50** (Key_P) ЛИБО **0x417** (U+0417 «З» — та же клавиша в
  русской раскладке!). `ScreenShotSave` @0x67aa60 = `SendToMainCtrl(5)` и НИЧЕГО больше —
  путь/формат снимка в этом классе не восстановимы.
- `KGlobalEventFilter` (QObject, sizeof 0x18): ловит только `MouseButtonPress`, дедуплицирует
  по 8 байтам `QEvent+0x18` (в раскладке Qt5 это modState+timestamp — одно нажатие приходит
  фильтру несколько раз), испускает `SignalGlobalMouseEvent` и НИКОГДА не съедает событие.
  Ctor заинлайнен в `KReportEditUi::InitConnect` @0x4d16e0; слушатель один — `OnGlobalMouseEvent`.
- `KLabelHoverEventFilter` (QObject, sizeof 0x28) — ⚠️ **МЁРТВ в сборке** (ссылок на vtable
  нет нигде, кроме собственных dtor). Портирован ради полноты: на `ToolTip` (0x6e) показывает
  подсказку и ПРОВАЛИВАЕТСЯ дальше; Enter(0xa)/Leave(0xb) живут только при смещении **<= 39**.
  Точка: `mapToGlobal(0,0) + (-10, -2*height + offset)`.
- `KTableWidget` @0x6876a0 (QTableWidget): ctor не настраивает НИЧЕГО, весь класс — одна
  инструкция в `keyPressEvent` @0x6876d8: `b QAbstractItemView::keyPressEvent`, то есть вызов
  **деда** в обход QTableView (гасит табличную навигацию стрелками/Tab). Настройки ставит
  владелец — `Ui_KImgList::setupUi` @0x681018 (`KlList_box`: обе полосы off, NoEditTriggers,
  tabKeyNavigation false, **ElideMiddle**, заголовки скрыты). Применено в нашем `KImgList`.
- `KPosNameLineEditDelegate` @0x5a8fb0 (QStyledItemDelegate + KObject): весь класс —
  `createEditor` @0x5a90b0 = QLineEdit + `setMaxLength(50)` + стиль
  «QLineEdit { background-color:#1a1a1a; outline: none;}»; валидатора НЕТ. Ставится в
  `KReportEditAddMarkView::InitPosName` @0x4bfa38 через `setItemDelegate` **на весь вид**.
  Вторая база KObject (SubscribeMsg 10001) — seam, обработчика у делегата всё равно нет.
- `KBackGround` @0x457cd8 (KDialog, sizeof 0x60): фон/пиксмап НЕ грузит вовсе — это пустой
  фуллскрин (SetKStyle(1)) без кнопки закрытия, закрывающий сам себя по QTimer(1000 мс).
  Точка входа `OpenBackGround()` @0x457fe8 в прошивке ⚠️ **никем не вызывается**.

**✅ KPatientListWidgetItem + KHoverEventFilter + setElidedFrame (2026-07-23).**
Self-test `patnavitem`, рендер — `patientmgmt` (лево-навигация теперь иконочная).
- Реф. `KPatientListWidgetItem : QFrame` @0x79e9c0 `(QMap<QString,QString>, QString, QWidget*)`,
  sizeof 0x60: `label_img` на ВЕСЬ фрейм QRect(0,0,211,80) AlignCenter + `label_text`
  QRect(23,50,165,28). Размер фрейма после чтения карты переопределяется **размером
  пиксмапа normalIcon** (setFixedSize). Ключи карты (порядок чтения): `selectIcon`,
  `normalIcon`, `hoverIcon`, `disableIcon`. Четыре состояния Select/UnSelect/Hover/Disable
  меняют ТОЛЬКО пиксмап (никаких stylesheet/флагов); единственный stylesheet — `SetFontSize`
  («QLabel{font-size: %1px;}», зовут с 15).
- Иконки: `KSystem::ProjectPresetPath()` + **`patient/mainicon/`** (@0x8a1ec0) —
  `{patientlist,examlist,report}_{select,normal,hover,disable}.png` (в поставке все 211×80).
- Тройка строк (реф. `InitListWidgetItem` @0x7a5e58): `TR_PInfo`+**`(F3)`**,
  `TR_PList`+**`(Alt+F3)`**, `TR_DQueue`+**`(F10)`**; SizeHintRole = QSize(211,80);
  первая строка `Select()`. ⚠️ В порте раньше стояли ДРУГИЕ ключи (TR_Ptnt/TR_Case) и не
  было суффиксов-горячих клавиш. Стиль списка тоже заменён на референсный (@0x8a1dc0, 250
  символов): у выделения и hover фон **прозрачный** — подсветку даёт сам item сменой пиксмапа.
- `setElidedFrame` @0x7a3468: если `width()-10 > ширина текста` — выход; иначе ставится
  `KHoverEventFilter` и текст = `elidedText(text, Qt::ElideRight, labelW - suffixW + 17)`
  + суффикс. Аргумент `int n` НЕ используется (зовут с 6).
- `KHoverEventFilter` @0x833678: на `QEvent::ToolTip` показывает полный текст в
  `mapToGlobal(0,0) + (-10, -2*h + 150)`; ветки Enter/Leave живут только при offset <= 39.
- Порт-решение: списку задана фикс-высота `count * 103` — без неё вьюпорт обрезал подпись
  ТРЕТЬЕЙ строки (поймано скриншотом, self-test этого не видел).

**✅ KImgListCell + встраивание в KImgList (2026-07-23).** Self-test `imglistcell`.
- Реф. `KImgListCell : QFrame` @0x6820f8 (sizeof 0x38) — КОМПОЗИТ, хотя API повторяет QLabel:
  QVBoxLayout(spacing 0, поля 0) + `label_img` (Preferred/Expanding, AlignCenter) +
  `label_txt` (AlignCenter). ctor: resize(400,300), StyledPanel/Raised, и ⚠️ **label_txt
  скрыт**; показывается только из `setText()` — а `setText` в прошивке не зовут нигде.
- ⚠️ КВИРК размеров: `setMin/MaxSize` режут картинке 20 px под подпись **только если она
  ВИДИМА** (реф. читает бит WA_WState_Visible). В self-test это ловится лишь у ПОКАЗАННОГО
  виджета — иначе ветка недостижима (первый прогон дал FAIL именно из-за этого).
- Реф. `Ui_KImgList::setupUi` @0x681018 держит `KTableWidget* KlList_box` + **ЧЕТЫРЕ**
  `KImgListCell` (KlList_img0..3). У нас превью-ячеек не было вовсе — добавлены, со стилем
  из `initImgInfo` @0x67d448 и размерами из icon-rect; появились `SetImgInfo`/`ClearImgInfo`.
- ⚠️ Дефолты прямоугольников были ВЫДУМАНЫ (190×191 / 188×135). Реф. @0x6508a8/@0x650ac8:
  `KImgList/tablecell` → QRect(0,0,269,209) = **270×210**, `KImgList/tableicon` →
  QRect(0,0,269,201) = **270×202** (координатная форма x1,y1,x2,y2 — размер на 1 больше).

**✅ ЖИВОЕ OSD-СЕМЕЙСТВО: KOsdSpinBase / KOsdSpin02 / KOsdReturnLabel + ПОЧИНЕНА БАЗА KFrame
(2026-07-23).** Self-test `osdspin02`, рендер — `osdsubmenu`/`irismenu`.
- ⭐⭐ **`KOsdSpin` (@0x484050) — МЁРТВЫЙ КОД ПРОШИВКИ.** Во всём `.text` НЕТ ни `bl 0x484050`,
  ни единого `adrp` на страницу `0x484000`, ни ссылки на его vtable `@0xa29e98` (проверено
  греп-ом полного дизасма). Единственный реально создаваемый спин — **`KOsdSpin02`**
  (`KOsdSubMenu::AddItem(_KOsdSpinConfig)` @0x47bd20). Наш порт полгода хостил неиспользуемый
  вариант. `KOsdSpin` оставлен как есть (историческая раскладка с кнопками «+»/«−»),
  хостится теперь `KOsdSpin02`. **ПРИЁМ:** прежде чем «улучшать» портированный класс —
  проверить греп-ом дизасма, что его вообще кто-то инстанцирует.
- `KOsdSpin02` (@0x484e18, sizeof 0x78): фрейм 250×32 фикс, grid (hsp 6, vsp 0, поля 9,0,9,0),
  только 2 ячейки — `label_title`(0,0) и `spin_value`(0,1) min-width **180**, NoFocus,
  Align 0x84, ⚠️ `setButtonSymbols(UpDownArrows)` — НАТИВНЫЕ стрелки видны (у KOsdSpin было
  NoButtons + свои кнопки). `AddValue/SubValue` — арифметика с клипом по max/min (у KOsdSpin
  был `btn->click()`). `SetIntValue` — тихий сеттер ЧЕРЕЗ ФЛАГ `m_notify` (не disconnect!)
  и он ОБНОВЛЯЕТ cur (KOsdSpin не обновлял). `ConfirmKeyActImpl` — пустой `ret`.
  Стили QSpinBox: focused `rgb(0,205,209)` (68), selected И unselected — ОДНА строка (70).
- `KOsdSpinBase` (@0x485570, абстрактный, 3 чисто виртуальных слота vptr+0x210/0x218/0x220):
  единственное поле `bool m_gotFocusIn @+0x42`. **ConfirmKeyAct = ТУМБЛЕР захвата фокуса**
  (`m_gotFocusIn ^= 1` → Focused()/Selected() → ConfirmKeyActImpl). Up/Down работают ТОЛЬКО
  при захвате. Именно это терялось, пока база была «сплющена» в QFrame.
- `KOsdReturnLabel` (@0x482440, ctor БЕЗ параметров, sizeof 0x50): 250×32, QHBoxLayout
  spacing 9 / поля (9,0,0,0), один `label_icon`. ⚠️ В ctor пиксмапа НЕТ — стоит текст
  `tr("icon")`; картинка (`return_normal.png`/`return_select.png` из qss/black/osd)
  появляется только из Selected/UnSelected. ConfirmKeyAct ИГНОРИРУЕТ аргумент и зовёт
  `KDialog::close()` меню-владельца. Создаётся ровно в `AddAReturnBtnInTheEndIfNeeded`
  @0x47bf60 (флаг ctor-а меню + защёлка), которую **InitWidget зовёт ПЕРВОЙ строкой**.
- ⚠️ **KFrame: раскладка полей была фиктивной** — реально `+0x30 _KPoint m_startPoint`,
  `+0x38 KOsdSubMenu* m_locatedMenu`, `+0x40 bool m_greyed`, `+0x41 bool m_selected`.
  И `Focused/Selected/UnSelected` у базы — **НЕ заглушки**: они ставят stylesheet
  (@0x868a30/a98/b00 по 98 символов и @0x868b68/ba8 по 63). Квирк вендора сохранён:
  в focused-строке цвет записан как `rgb(0,  205,209)` — ДВА пробела. Добавлены
  `IsGreyed/SetGreyed/IsSelected/UpdateUI` (@0x47f888: selected ? Selected : UnSelected).
- Навигация `KOsdSubMenu` приведена к реф.: Up/Down @0x47b198/@0x47b118 сначала спрашивают
  у текущей строки `GotFocusIn()` (захваченный спин ест клавиши сам), **заворачивание
  курсора живёт в `RefreshSelectedItem` @0x47ae30** (>=count → 0, <0 → count-1) вместе с
  проходом Selected/UnSelected по всем строкам и пересчётом sizeHint; `ConfirmKeyAct`
  @0x47b080 зовёт **Focused()** (vt+0x1a0), а не Selected (у нас было неверно), и передаёт
  ИНДЕКС строки; `keyPressEvent` @0x47b218: Up/Down/Right/Return/Enter, а Left/Escape —
  «отпустить захваченный спин, иначе close()». Гейт `isActiveWindow()` во всех трёх.
  Хвост InitWidget: `InitItemPosition` → `RefreshSelectedItem` (⚠️ без этого строка «назад»
  так и осталась бы с текстом «icon» вместо иконки — поймано скриншотом).

**✅ KFlexEndoBtnGuide + KUserSet::GetFunctionName (2026-07-23) — UI-бэклог вскрыт заново.**
⭐ **ПРИЁМ ИНВЕНТАРИЗАЦИИ UI (после того как «виджет-слой исчерпан»):** класс-QObject виден
по символу `_ZN<len><Class>16staticMetaObjectE`. Полный список непортированных:
`nm update/root/X2000 | grep -o '_ZN[0-9]*[A-Za-z_0-9]*16staticMetaObjectE' | sed -E
's/^_ZN[0-9]+//; s/16staticMetaObjectE$//' | sort -u` → каждое имя грепать `class <N>` в
app/src. Из 259 QObject-классов бинарника НЕ портированы (за вычетом Qt-шных):
KBackGround, KCalendarTextNavigator, KCalendarView, KDICOMSrvListItem, KEventFilter,
KGlobalEventFilter, KHoverEventFilter, KLabelHoverEventFilter, **KFlexEndoBtnGuide**(сделан),
KImgListCell, KOsdReturnLabel, KOsdSpin02, KOsdSpinBase, KPatientListWidgetItem,
KPosNameLineEditDelegate, KPoweroff, **KRecordItem**, KTableWidget,
KToProgressDlgMsgDispatcher, report_template::KLineEdit (+ не-UI: ColdLight, HmiMcu,
KAutoTestThread, KSelfTest, KTimeMng, KUpdateMng, OperateWaitThread). Это очередь фазы F.

- `KFlexEndoBtnGuide` (`ui/KFlexEndoBtnGuide.{h,cpp}`, превью `flexguide`, self-test `flexbtn`).
  Гайд кнопок ПУЛЬТА ДУ гибкого эндоскопа. У класса **НЕТ `Ui_`-структуры** (в отличие от
  жёсткого KRigidEndoBtnGuide): всё строит код — `InitWidgets` @0x69ece0 = 4 × `InitItem`
  @0x69e860 + хвостовой слот. Строка i: иконка 28×28 в **(0, 36·i)** (реф. арифметика
  `(i + (i<<3)) << 2`), подпись 150×28 в **(36, 36·i)**, `AlignLeft|AlignVCenter` (0x81).
  Пиксмапы `qss/black/icon/endobtn/btn0..3.png` (исходники 28×**29** — реф. масштабирует
  `IgnoreAspectRatio`/`FastTransformation`, т.е. НАМЕРЕННО с искажением). Стиль иконки —
  литерал @0x867840 «QLabel { background:transparent; border: none }». Фона нет вовсе.
  Хранилище — `unordered_map<int,pair<QLabel*,QLabel*>>` (единственное поле, +0x30).
  Геометрия самого виджета — НЕ в классе: её ставит владелец `KViewSoftEndo::InitUiConfig`
  @0x46d7f0 из `KDisplayOption::GetSoftEndoViewConf()` + `font-size:16px;` + objectName
  `widget_endobtnguide`; в порте канва 186×136.
  `Show(bool)` @0x69e5b0: `!bShow || !IsEndoReady()` → hide; иначе show и **строки 0 и 3
  скрываются на модели `EUD.100S`** (@0x862990) — у неё две кнопки вместо четырёх.
- ⚠️ **`KUserSet::GetFunctionName` @0x661b88 — ЭТО ДРУГАЯ ТАБЛИЦА, не `KUserOsdSet`!**
  Легко было бы переиспользовать существующую и получить неверные подписи. 11 записей,
  иной порядок ID: 0 `TR_Frz`, 1 `TR_Zm1`, 2 `TR_LMode`, 3 `TR_IRIS1`, 4 `TR_IEnh`,
  5 `TR_CEnh`, 6 **`"CHb"` — ЛИТЕРАЛ** (`fromAscii_helper(@0x889b80, len 3)`, БЕЗ tr!),
  7 `TR_Ctrst`, 8 `TR_AGC1`, 9 `TR_Snp`, 10 `TR_Rcd`; промах → ПУСТАЯ строка. Строки —
  адреса в `.rodata` (прочитаны напрямую через ELF-таблицу секций). Self-test `flexbtn`
  специально проверяет, что таблицы двух классов РАЗЛИЧНЫ.
- ID функций строк = `user.ini` **[RemoteSwitch]Switch1..4** (реф. поля `_KUserConf`
  +0x00..+0x0c, ключи «RemoteSwitch/Switch1..4» @0x889bd0/be8/c00/c18 в
  `KUserSet::ReadVideoParamConfig` @0x663168). У нас их уже читает `KRemoteSwitchConfig` —
  переиспользован, дубля не заведено. На поставке: 0/9/4/2 → Frz/Snp/IEnh/LMode (скриншот).

**✅ ОСТАЛЬНЫЕ ЧЕТЫРЕ Te-ТВОРЦА (2026-07-23) — семейство Te закрыто.**
`KRTTeTextGroupCreator` / `KRTTeImageItemCreator` / `KRTTeImageGroupCreator` /
`KRTTeSubDataItemCreator` в `report/KRTTeItemCreators.cpp`.
- ⚠️ **РАЗНЫЕ РАЗДЕЛИТЕЛИ у соседних творцов** (оба сверены чтением .rodata и видны рядом
  на скриншоте): TextGroup клеит «label` : `value» — **ТРИ** символа (@0x8750e8), а
  текстовый творец — «title` :  `» с **ЧЕТЫРЬМЯ** (@0x875108). Унификация была бы ошибкой.
- `TextGroupCreator` @0x519a78: слот 5 `GetTextGroupData` → `InsertTableTitle` →
  `CreateTable` → таблица null/1×1 → `CreateChild(в ячейку)` @0x519430 (всё подряд в ОДНУ
  ячейку, БЕЗ char-формата), иначе `CreateChild(в таблицу)` @0x5196c0 (`resize` под число
  пар, раскладка по сетке И char-формат из `GetFontSize`/`GetFontColor` с **item == nullptr**
  — дефолтный шрифт, не item-специфичный). Возврат — ЧИСЛО ПАР, не 0/1.
- `ImageItemCreator` @0x514870: к источнику данных НЕ ходит; размеры картинки —
  значение атрибута **делится на `KScreenMng::GetRatioTo1K()`** (масштаб под разрешение);
  выравнивание Left/Right или центр по умолчанию; заголовок вставляется ПЕРЕД картинкой,
  но НЕ при значении **`NoPFData`** (@0x874e10). Свой `GetItemTitle` @0x515270 — ДВА гейта
  (режим == 2; `m_strShowTitle == "1"`), оба ведут к пустой строке. Возврат 1.
- `ImageGroupCreator` @0x514410: слот 7 `GetImageGroupData`; `CalcmageWidth` @0x514200 —
  **600.0 / table->columns()** (константа подтверждена; реф. имеет `assert(columns > 0)`),
  атрибут-переопределитель есть, но его ключ НЕ ВОССТАНОВЛЕН. Две перегрузки `CreateChild`:
  @0x513b70 — все картинки в одну ячейку, @0x513ea0 — `resize` под `ceil(count/cols)` и
  раскладка по сетке.
- `SubDataItemCreator` @0x515510: слот 8 `GetSubData`; при НЕПУСТОМ результате — конфиги
  вложенного шаблона регистрируются через `AppendItemParam`, затем табличные `CreateChild`
  (унаследованы, не переопределены). ⚠️ При ПУСТОМ результате и режиме != 2: если
  `m_strShowTitle == "1"` → просто `return 1` БЕЗ контента; иначе вставляется один блок
  с `m_strTitle` как обычный текст.
- НЕ ВОССТАНОВЛЕНО: ключи атрибутов ширины/высоты/выравнивания картинки и ключ
  переопределения ширины в `CalcmageWidth` — это статические `std::string` в .bss ДРУГОЙ
  таблицы (не @0xa86098), их текст статически не читается ⇒ вынесены в
  `SetImageAttrKeys` / оставлены дефолтом 600.
...
- `CreateItem` @0x515ec0 — порядок строгий: `CheckCreate` → false → 0;
  `InsertTableTitle` БЕЗУСЛОВНО (до таблицы); `CreateTable`;
  ⚠️ **таблица == nullptr → `CreateChild` в ИСХОДНУЮ ячейку и НЕМЕДЛЕННЫЙ выход**
  (разделительная линия в этой ветке НЕ вставляется);
  таблица 1×1 → `CreateChild` в `cellAt(0,0)`; иначе → `CreateChild` по сетке;
  затем `GetSplitLineInfo(item.m_strID, конфиги)` и при ширине > 0 — `InsertSplitLine`.
- `CreateTable` @0x5176f8: столбцов — `m_strColumn.toInt()`. Для имён
  **HOSPITAL_OTHER** (@0x875070), **RT_SIGNATURE** (@0x875080), **RT_ADDITION**
  (@0x875090), **RT_IMAGE_TEXT_MAP** (@0x8750a0) таблица строится ВСЕГДА; для прочих при
  одном столбце возвращается **nullptr** — и тогда дети идут прямо в исходную ячейку.
  Ширины столбцов — строка вида «25,50,25», режется по «,» (@0x863930) в проценты;
  иначе равные доли 100/cols. `cellPadding = 0.0` (property 0x4103).
- Две перегрузки `CreateChild`: @0x515d90 — линейно параграфами в ОДНУ ячейку, между
  удачными детьми `insertBlock`, в конце `deletePreviousChar` (убирает хвостовой перевод);
  @0x5160c8 — раскладка по сетке `row = i / cols`, `col = i % cols`, в конце `removeRows`
  для лишних строк.
- `GetItemTitle` @0x5185a0 — гейт ЖЁСТЧЕ, чем у текстового творца: заголовок отдаётся
  ТОЛЬКО при `m_strShowTitle == "1"`. ⚠️ Литерал `"1"` получен РАЗДЕЛЕНИЕМ СУФФИКСА строки
  `" > /dev/null 2>&1"` (@0x885600) — адрес выглядел подозрительно, проверен чтением
  .rodata (тот же приём, что с `PatientID` внутри `IssuerOfPatientID`).
  При пустом `tr(m_strTitle)` — фолбэк на `QueryTemplateItemRealTitle`.
- НЕ ВОССТАНОВЛЕНО и помечено: ~14 ключей атрибутов из статической таблицы @0xa86098
  (это динамически инициализируемые глобальные `std::string`, а не литералы .rodata) —
  в т.ч. ключ списка ширин столбцов (вынесен в `SetColumnWidthAttrKey`, по умолчанию
  равные доли) и ключи межстрочного интервала в `InsertTableTitle` (LineHeightType 2 vs 4);
  формула числа строк (реф. использует поле item+0xf0 и «строк заголовка» из под-конфига —
  в порте прямая раскладка детей по сетке); ветка `GetItemTitle` при режиме контекста == 2.
...
он выполняет реф.-последовательность @0x4fdce8 (Reset параметра Simple → `Simple::Display`
→ `Te::Reset` → передача набора валидных в Te → `Te::Display(data, printer)` → печать
документа). Прежний html-стаб оставлен ФОЛБЭКОМ на случай неподключённого конвейера.
Диалог получил поля обоих движков (реф. simpleDisp +0x68, teDisp +0x70) и
`SetReportSource(ds, data)` / `Context()` для регистрации творцов.
⭐ Скриншот подтверждает архитектуру НАГЛЯДНО: в шаблоне пять элементов, но у `/biopsy`
данных нет — Simple не кладёт его в набор валидных, `CheckCreate` в Te его отбрасывает,
и `removeRows` убирает лишнюю строку. В отчёт попали ровно четыре.
⚠️ **ОШИБКА ПОРТА, ПОЙМАННАЯ СКРИНШОТОМ:** первая версия `KRTTeCreatorContext::Reset()`
чистила реестр творцов. Реф. наполняет его ОДИН РАЗ (`InitCreator` @0x50c9d0 из ctor),
а `Reset` зовётся ПЕРЕД КАЖДЫМ рендером — очистка делала движок пустым, и отчёт выходил
белой страницей. Исправлено; реестр в Reset не трогается.
⚠️ Te-творец текстового блока в превью — НАША минимальная реализация (тела реф. Te-творцов
ещё не реверсированы), помечено в коде.
...
Стал возможен сразу после снятия C8. Реф.-факты (сверены дизасмом):
- `KRTSimpleDisplay : KRTAbsDisplay` (ctor базы @0x506928), sizeof 0xE8. Layout:
  vptr + `KRTAbsDataSource*` +0x08, **`KReportDisplayParam` ВСТРОЕН ПО ЗНАЧЕНИЮ** +0x10
  (ровно 0xD0 байт — сходится с его sizeof), `KRTSimpleCreatorContext*` +0xE0 (владеющий).
  Контекст получает УКАЗАТЕЛЬ на встроенный параметр и им НЕ владеет.
- `Display` @0x50a308 — вся функция: `UpdateTemplateDisplayParam(data.m_mapConfigs,
  data.m_mapItemConfigs)`, затем цикл по `data.m_lstItems` → `CreateItem(item, params)`,
  где `params` — ЛОКАЛЬНАЯ мапа, которая НИКОГДА не заполняется (уходит пустой на каждый
  элемент), результаты не агрегируются, `return true`. `Reset` @0x50a248 — только сброс
  встроенного параметра.
- `InitCreator` @0x508160 — **ДЕВЯТЬ** пар «ключ → творец» (у «взрослого» `KRTCreatorContext`
  @0x547918 их шесть). Отличия Simple-версии: появляется класс `KRTSimpleTextGroupCreator`
  (аналога во «взрослом» НЕТ), а `KRTSimpleTableItemCreator` регистрируется под ЧЕТЫРЬМЯ
  ключами (добавлены `RT_ROW_TABLE_BLOCK`, `RT_OB_Z_SCORE_BLOCK`) — и это ЧЕТЫРЕ ОТДЕЛЬНЫХ
  объекта (4 разных `operator new`), а не один общий; подтипа он, в отличие от «взрослого»,
  не принимает.
- `CreateItem` @0x509350 сам НИЧЕГО не создаёт — готовит «измерения повтора»: ищет
  `item.m_strID` в item-конфигах параметра, в найденном конфиге ключ `"RepeatCommand"`
  (@0x865c98), разбирает его через `RevertPathByID(значение, "RefColumnID" @0x865c88)`,
  затем для атрибутов, которых ЕЩЁ НЕТ во входящем `params`, спрашивает варианты у
  источника данных — и делегирует в `HandleRepeat`.
  ⚠️ **ИСПРАВЛЕНО ПРОТИВ ОТЧЁТА РЕВЕРСА:** субагент написал «виртуальный слот 4 у item»,
  но у `KReportTemplateItem` виртуальных методов нет. Проверено отдельно: получатель —
  `m_pDataSource` (`ldr x4,[x28,#8]` @0x509534), слот +0x20 = **`GetTextOptional`**,
  первый аргумент `item+0x80` = `m_strDataSrc` (`add x1,x25,#0x80` @0x509548).
- `HandleRepeat` @0x508c00 — РЕКУРСИЯ, дающая **ДЕКАРТОВО ПРОИЗВЕДЕНИЕ**: пустой список →
  найти творца по `item.m_strType` и вызвать его (нет в реестре → 0); иначе снять ПЕРВОЕ
  измерение и для КАЖДОГО его варианта положить `params[ключ] = вариант` и рекурсивно
  вызвать себя с остатком, **СУММИРУЯ** результаты. При N ключах творец вызывается
  M1×…×Mn раз. Именно это и проверяет self-test (2×3 → ровно 6 вызовов, 6 разных комбинаций).
- Базовый `KRTSimpleAbsItemCreator::CreateItem` @0x50a3f8 — `assert(false)` ⇒ в порте
  чисто виртуальный (тот же приём, что с `KRTAbsDataSource`).
- ✅ **ТЕЛА ШЕСТИ ТВОРЦОВ ПОРТИРОВАНЫ** (2026-07-23, self-test `rtcreators`).
  Собственных полей у них НЕТ — всё через ссылку на контекст (реф. `this+8`).
  Общий пролог: поля `m_strID`/`m_strDataSrc` прогоняются через
  `report_template::ConvertToSourceID(поле, params, out)` ТОЛЬКО при непустом `params`.
  Общий эпилог при успехе: `KReportDisplayParam::AppendValidItem(id)`.
  ⭐ **ГЛАВНОЕ ПОНИМАНИЕ:** результаты `Get*Data` реф. НИГДЕ НЕ СОХРАНЯЕТ — буфер
  заполняется и тут же освобождается. Simple-движок не собирает контент, он вычисляет
  ДОСТУПНОСТЬ элементов. Это и объясняет замеченную ранее странность, что `IsItemValid`
  читает ДРУГОЕ множество: набор «валидных» наполняется здесь для ПОТРЕБИТЕЛЯ
  (второго движка отображения), а не для себя.
  Асимметрии по творцам (легко потерять при «упрощении»):
  • `Text` @0x50b408 (слот 3 GetTextData): ПУСТОЙ `m_strDataSrc` → источник НЕ опрашивается,
    элемент СРАЗУ валиден и возврат **1**.
  • `TextGroup` @0x50b158 (слот 5): пустой `m_strDataSrc` → **0**. ОБРАТНАЯ асимметрия.
  • `Image` @0x50a758 (слот 6): сперва берётся ПОСЛЕДНИЙ токен `m_strID` по разделителю
    `'/'` (иммедиата 0x2f) и ищется в статической карте локальных картинок
    `MAP_LOCAL_PIC_PFINFO` @0xa78f98 — найден → валиден и 1 БЕЗ обращения к источнику.
    Содержимое карты НЕ ВОССТАНОВЛЕНО ⇒ инъектируемый набор `SetLocalPicNames`.
  • `ImageGroup` @0x50a4a8 (слот 7): как TextGroup.
  • `Table` @0x50af00: к источнику НЕ обращается ВООБЩЕ, `m_strDataSrc` не читает;
    рекурсивно прогоняет `m_lstSubItems` через контекст и возвращает **СУММУ**, а не 0/1.
  • `SubData` @0x50ac20 (слот 8 GetSubData): id берётся из **`m_strDataSrc`**, НЕ из
    `m_strID`; при успехе переносит `m_mapItemConfigs` вложенного шаблона в
    `KReportDisplayParam::AppendItemParam` и рекурсивно строит вложенные элементы,
    возвращая **1 + сумму**.
- НЕ ВОССТАНОВЛЕНО: call site создания `KRTSimpleDisplay` и логика выбора Simple vs
  `KRTTeDisplay` (второй наследник `KRTAbsDisplay`, у него есть `Display(data, QPrinter*)`).

**✅ СНЯТА КОЛЛИЗИЯ C8 — `KReportDisplayParam` (2026-07-23).** Разрешена СОВМЕЩЕНИЕМ:
реф. методы носят ТЕ ЖЕ ИМЕНА и отличаются только типами аргументов (std::string/set/map
вместо QString/QSet/QVector) ⇒ добавлены ПЕРЕГРУЗКАМИ в тот же класс, прежний упрощённый
слой не тронут (self-test `dispparam` расширен, а не переписан).
Реф.-факты (сверены дизасмом) — они СИЛЬНО расходятся с тем, что мы придумали:
- Реф. класс — **ПАРАМЕТР-КОНТЕЙНЕР, а не вычислитель**. sizeof 0xD0: флаг +0x08,
  `map<string,string>` +0x10, `map<string,ItemConfig>` +0x40, `set<string>` валидных +0x70,
  `set<string>` референсных +0xa0.
- `UpdateTemplateDisplayParam` @0x507110 — РОВНО два `operator=` и больше ничего: ни разбора
  ключей, ни разделителей, набор валидных не пересчитывается. Наша рекурсивная `markItem`-
  модель — САМОСТОЯТЕЛЬНАЯ выдумка (оставлена как отдельный слой, помечена в коде).
- `AppendValidItem` @0x506ca0 — БЕЗ гейта по ref-множеству и возвращает **void**
  (в функции нет ни одного обращения к +0x08/+0xa0).
- `IsItemValid` @0x506a38 читает **референсный** набор (+0xa0), а НЕ тот, что наполняет
  `AppendValidItem` (+0x70). Краевой случай АСИММЕТРИЧЕН и легко теряется при «упрощении»:
  флаг ни разу не взводился → **всегда true**; набор задан, но пуст → **всегда false**.
- `SetRefValidItems` @0x506ec0 — присваивание + безусловный взвод флага.
- `AppendItemParam` @0x506ef0 — **MERGE** (существующий ключ пропускается),
  в отличие от `UpdateTemplateDisplayParam`, который **ЗАМЕЩАЕТ**.
- `Reset` @0x506c10 чистит ВСЁ состояние (оба map, оба set, флаг), а не только «валидные».
- Строковых литералов в классе НЕТ ни одного (проверено по всем 8 функциям).
**➡️ Блокировка «Simple»-движка отчёта СНЯТА:** `KRTSimpleDisplay` (@ctor 0x50a2b8,
Display/InitMembers/Reset) и `KRTSimpleCreatorContext` (@ctor 0x508b50, InitCreator/
HandleRepeat/CreateItem) принимают `KRTAbsDataSource*` и `KReportDisplayParam*` — оба уже
есть. Это следующая цель ветки отчёта.

**✅ UI: ПОРТИРОВАН `KVideoLabel` (2026-07-23) — ВИДЖЕТ-СЛОЙ ИСЧЕРПАН.**
`app/src/ui/KVideoLabel.*`, self-test `videolabel`, превью `videolabeldraw`.
Оверлей поверх области видео; владелец — `KViewSoftEndo::UpdateVideoLabel(QRect)` @0x467368
(рассылает геометрию ему, `KFloatingMsg` и `KMsgPopup`). Реф.-факты:
- `KVideoLabel : QLabel`; ctor @0x6876e0: `setAutoFillBackground(true)`,
  `setWindowOpacity(1.0)`, `setAttribute(120 = Qt::WA_TranslucentBackground, true)`;
  m_videoRect (+0x30) НЕВАЛИДЕН, m_imagePath (+0x40) пуст, режим (+0x48) 0, t (+0x50) 0.
  `setRenderHint(Antialiasing)` НЕ включается нигде.
- Перо везде одинаковое: `QPen(QBrush(цвет, SolidPattern), 1.0, SolidLine, SquareCap,
  BevelJoin)`. Цвета — сырые Qt::GlobalColor: **7 = red** (дуги/линии/флаги),
  **9 = blue** (`PaintVideoLabel`).
- `DrawLine(int, double)` @0x687d78 целиком: `режим = a; t = d*0.5 + 0.5; update()` —
  т.е. наружу приходит «сырой» D ∈ [-1,1], а хранится t ∈ [0,1].
- `paintEvent` @0x688328: guard по невалидному m_videoRect → выход; image-view рисует
  картинку по центру ВНЕШНЕГО прямоугольника (+0x28), иначе `eraseRect(m_videoRect)`;
  далее по режиму 1 (срез углов, 8 линий из device-данных) или 2 (дуги + двойные линии).
- `ArcSpans` (реф. внутри `PaintDoubleArc` @0x6877d8): круг радиуса d в центре (w/2, h/2);
  w==h → одна дуга (0, **5760** = 360°×16); w>h → две дуги
  start=(a+90)·16 и (a−90)·16, span=(180−2a)·16, где a=acos((h/2)/d) в градусах;
  w<h → start=a·16 и (a+180)·16 с a=acos((w/2)/d). π — литерал @0x88b050.
- ⚠️ **ИСПРАВЛЕНА ОШИБКА В ОТЧЁТЕ РЕВЕРСА.** Для `PaintDoubleLine` @0x6880b0 субагент дал
  `sqrt((сторона/2)² − d²)` — операнды переставлены. В бинарнике `fnmsub d0, d1, d1, d0`
  @0x688188 (d1 = радиус, d0 = (сторона/2)², деление на 2 видно по
  `add w21,w21,w21,lsr #31; asr w21,#1` @0x688170) даёт **sqrt(d² − (сторона/2)²)**.
  Поймано СКРИНШОТОМ: с переставленными операндами корень отрицателен, линии не рисуются
  и контур разомкнут; с правильными — дуги и линии смыкаются в «обрезку углов».
  Геометрия сходится: круг радиуса d достаёт до боковой грани только при d > сторона/2.
- `DrawFlag` @0x687d90 — пять крестиков ±10 СВОИМ пером не рисует (берёт текущее):
  (A) целочисленный в центре внешнего прямоугольника (+0x28), (B) в
  lerp(topLeft, bottomRight, 1−t), (C) со сдвигом width·D, (D) со сдвигом height·D,
  (E) в lerp(…, t), где D = 2t−1.
- НЕ ВОССТАНОВЛЕНО: сеттер поля +0x28 (в порте заведён явный `SetMarkerRect`);
  вызывающий код `PaintVideoLabel` (из `paintEvent` она НЕ зовётся).
- DEVICE-STUB: `KVideoSet::GetCornerCuttingInfo()` → инъектируемый провайдер;
  `KDisplayOption::getVideoRectForImgPro()` берётся настоящий (он уже портирован).
...
подставленный `QLineEdit` (реф. `KPinyinWidget::InitUI` @0x763db8 создаёт именно его).
Класс восстановлен ЦЕЛИКОМ (все 7 методов):
- ctor @0x763d80 — ровно `QLineEdit(parent)` + vptr, никаких свойств.
- `text`/`insert`/`setText` @0x764220/@0x764248/@0x764250 — прямое проксирование в базу
  (два последних — хвостовые вызовы).
- `focusOutEvent` @0x764218 — **хвостовой вызов `QWidget::focusOutEvent`, т.е. реализация
  QLineEdit СОЗНАТЕЛЬНО пропускается**: уход фокуса на попап кандидатов не сбрасывает
  выделение и не завершает ввод.
- `paintEvent` @0x765ca0 — заливает `cursorRect()` кистью `QBrush(Qt::white, SolidPattern)`
  и ТОЛЬКО ПОТОМ зовёт базу (порядок сверен: fillRect @0x765d14 → база @0x765d20).
  База перерисовывает поверх, так что видимого эффекта почти нет — воспроизведено дословно,
  реф. не «исправлялся».
Скриншот режима `pinyin` сверен — регресса нет.

**➡️ ОСТАТОК UI (по инвентаризации 2026-07-23):** все **76** классов с `Ui_*::setupUi`
портированы; виджет-слой ИСЧЕРПАН: по `paintEvent`/`resizeEvent` непортированных классов не осталось
(`KViewBase` и `KVideoLabel` закрыты 2026-07-23, см. выше).
...
✅ KGridWidget ПОРТИРОВАН (режим `gridwidget`, 80-й UI-класс): контроллер переукладки сетки (реф. @0x5a8350, base QWidget). Ранее подставлялся голым QWidget+QGridLayout. КЛЮЧЕВОЕ: НЕ авто-флоу и НЕ владеет layout'ом — обёртка над ВНЕШНЕ заполненным QGridLayout: caller строит сетку, отдаёт layout, KGridWidget снимает снапшот и ПЛОТНО переукладывает видимые ячейки, убирая дырки. InitWidget @0x5a8888 (фон-ребёнок «KGridWidgetBackground» lower() + SetGridLayout + SetColumn(деф.4) + LoadAllWidgets). LoadAllWidgets @0x5a8780: снапшот всех ячеек в vector<{widget,visible}>. MarkWidgetVisible @0x5a83a0 (только флаг). Relayout @0x5a86b8: RemoveAllWidgets + плотная укладка видимых (row=i/cols, col=i%cols, i по видимым) + show/hide. ShowAllRows/HideSpecificRows(fromRow). resizeEvent @0x5a89b8: фон под размер. Сигналов/выделения/paint НЕТ. 100% PORT. Скриншот сверен (7 кнопок 3 колонки, скрыл C3 → C4 занял место C3, дырка закрыта: row0 [C1 C2 C4] row1 [C5 C6 C7]).
✅ KExamListSearch ПОРТИРОВАН (режим `examlistsearch`, 85-й UI-класс) — ПЕРВЫЙ full-fidelity вью на РЕАЛЬНЫХ кастом-виджетах (KMemComboBox + KPatientDateEdit, без подстановок): панель поиска списка осмотров (реф. @0x7f42d8, base QWidget, setupUi @0x7f6a68). Сиблинг KPatientListSearch (реф. абсолютная геометрия; в порте QGridLayout как у сиблинга). Поля: edit_patientid(PatientID) / edit_name(PatientName) / cmb_doctor(KMemComboBox, DrExamName, setMaxLength(50)+SetTableName tb_QuickInputDoctor) / cmb_status(ReportStatus enum: «»/TR_All/TR_EXMING/TR_Exmed/TR_Cexam/TR_Dgnsed/TR_Pnted/TR_Uplded/TR_PUploaded) + ExamDate-диапазон (2 KPatientDateEdit dateEdit_start/end + btnStart/EndDate календарь, сепаратор «--»). Кнопки TR_Sch(F11)/TR_Rst2(F12). SlotToStartSearch @0x7f4788 собирает map условий (PatientID/PatientName/DrExamName like, ReportStatus=, ExamDate>=/<=) → emit QueryItems(map) (DB-seam — запрос выполняет вью). SlotToResetSearchData очищает. OnStart/EndDateChanged — кросс-ограничение from≤to (ГРАБЛИ: setMinimumDate сбивает пустой KPatientDateEdit с плейсхолдера → ограничиваем ТОЛЬКО если целевая дата не пуста). БОНУС: добавлен KMemComboBox::setMaxLength (реф. @0x690cd8, был пропущен). 100% PORT (QueryItems + tb_QuickInputDoctor — seam). Скриншот сверен (все поля + 2 date-плейсхолдера YYYY-MM-DD + doctor-combo dropdown + кнопки).
✅ KDicomQueueSearch ПОРТИРОВАН (режим `dicomqueuesearch`, 86-й UI-класс): панель поиска очереди DICOM (реф. @0x808f40, base QWidget, setupUi @0x80b198). Близнец KExamListSearch, но: НЕТ поля врача/KMemComboBox; ДОБАВЛЕН DICOM cmb_msgtype (TR_Tpe2, CommandType: idx2..4→MPPS/Store/Commitment); cmb_status→CommandStatus (idx2..5→Successful/Fail/In progress/Waiting); диапазон дат по LastUpdateDate (label TR_STime2). SlotToStartSearch @0x809028: SQL-фрагменты (PatientID/PatientName like, CommandType/CommandStatus COLLATE NOCASE =, LastUpdateDate >=/<=) склеены « and » → emit QueryItems({"Where": clause}) (DB-seam, отличие от многоключевого map KExamListSearch). Reset/OnStart-EndDateChanged (та же грабля кросс-ограничения). 100% PORT. Скриншот сверен (TR_Tpe2 вместо TR_Dctr, TR_STime2, 2 date-плейсхолдера, кнопки).
✅ KPageLineEdit ПОРТИРОВАН (режим `pagelineedit`, 87-й UI-класс): поле номера страницы пейджера (реф. @0x7b6990, base QLineEdit). НЕ контейнер — только editable-бокс текущей страницы. m_value(+0x30) — закоммиченная страница. SetMaximumValue @0x7b6b20 пересобирает валидатор под [1,max]. setText override @0x7b57e0: trimmed().toInt()→m_value + база. focusOutEvent @0x7b6c68: пустое/расхождение → revert к number(m_value). Собств. сигналов НЕТ (владелец слушает returnPressed/editingFinished). РЕФ. использует сгенерированный range-regex GetRegFromNumber в QRegExpValidator; в порте QIntValidator(1,max) (реверс подтвердил эквивалентность — точную побайтовую regex-генерацию не воспроизводим). mouse-drag-select покрыт штатным QLineEdit. 100% PORT. Скриншот сверен (Page [3] /50).
✅ KLayOut ПОРТИРОВАН (режим `layout`, 88-й UI-класс): ВНИМАНИЕ — НЕ селектор раскладки (имя обманчиво), а переиспользуемый chrome панели с заголовком (реф. @0x68bd20, base QWidget, setupUi @0x68c048, 386×300). Дерево: verticalLayout → QFrame#KFBackground (StyledPanel/Raised) → verticalLayout_2 → QFrame#KFUpbar (minSize 0×35) с horizontalLayout(15,0,15,0): label_title + btn_close(QToolButton, border0/transparent, iconSize 32, autoRaise) + вертик. спейсер-тело. KFUpbar/KFBackground — НЕ классы, objectName-стилизуемые QFrame. ctor: btn_close скрыт по умолчанию, 2-состояний иконка close.png/close_check.png (theme::asset black/icon/), connect clicked→clickBtnClose(). API: setTitle/setNoFrame(border0)/setNoCloseBtn(bool). DEVICE-SEAM: только KDisplayOption::GetIcon → 2-state QIcon. 100% PORT. Скриншот сверен (Panel Title + close-иконка ⊗ + тёмный фон). ВАЖНО: KBackGround @0x457cd8 — НЕ связан (image-editor KDialog), не путать.
✅ KPatientListViewUi ПОРТИРОВАН (режим `patientlistview`, 89-й UI-класс) — ⭐ КУЛЬМИНАЦИЯ семьи patient: родительский вью, собранный из РЕАЛЬНЫХ портированных виджетов сессии (реф. @0x7e5650, base QWidget, setupUi @0x7e7738, фикс 1630×1024). Коррекция скана: ОДНА KTableView (не 5), ЧЕТЫРЕ KPagePushButton (не 17). Скелет: QVBoxLayout → QGroupBox#grp_view → QVBoxLayout(18,4) → [widget_search (встроенный KPatientListSearch, reparent) + tableView (KTableView, 1591w) + widget (пейджер-бар, абсолютная геометрия)]. Пейджер-бар: label_db_record(20,1)«TR_Ttl:count» / label_hint(182,16)«TR_DCOPETExam\r\n\r\nF4/F5: TR_SCAPage» / btn_home(767,10) / btn_pre(835) / edit_page(911, KPageLineEdit, align right) / label_page(980)«maxPage» / btn_next(1021) / btn_tail(1090) — 4 KPagePushButton InitButton из ProjectPresetPath+«patient/pageturning/page_{head,front,next,tail}_{normal,hover,disable}.png». InitTable: заголовки tr() (sel/TR_PID/Nme/Gdr/Age/EItem/Aplct/PDate/ADate/Stts), col0 чекбокс, InitTableView(h,«PID»,15,0). RefreshPageInfo: метки + enable/disable кнопок по границам. gotoPage(head/pre/next/tail/custom из edit_page.returnPressed). DEVICE-STUB: OnGetPatientListDataFromDB → инъектируемый SetPageProvider (реф. KPatientListDBTableHandler::GetPageRecordFromDb + KObject IPC SubscribeMsg 0x2ee1..5); реф. навигация — eventFilter трекбол/hw-клавиши, в порте штатные clicked. KPatientListOptUi — сиблинг (не встраивается). Скриншот сверен (встроенный поиск + таблица с 5 фикстур-строк + пейджер 4 реальные иконки |◀◀▶▶| + edit «1» + «3» стр + TR_Ttl:15).
🔼 KExamListViewUi АПГРЕЙД до полной точности (режим `examlist`, НЕ новый класс — переписан прежний подстановочный порт): теперь embedded РЕАЛЬНЫЙ KExamListSearch + KTableView (20→показаны 10 колонок: sel/PID/Name/Gender/Age/ExamDate/Application/Doctor/EndoModel/EndoSN/Status) + пейджер из 4 KPagePushButton (patient/pageturning ассеты) + KPageLineEdit. SetPageProvider (DB-фикстуры), gotoPage/RefreshPageInfo как у KPatientListViewUi. Скриншот сверен (поиск + 4 фикстур-строки EC-X20L + пейджер |◀◀▶▶| + edit«1»/«2» + TR_Rcds:8). ПРИЁМ: прежние подстановочные вью можно апгрейдить реальными виджетами (KExamListViewUi — первый).
🔼 KDicomQueueViewUi АПГРЕЙД до полной точности (режим `dicomqueue`, НЕ новый класс): embedded РЕАЛЬНЫЙ KDicomQueueSearch + KTableView (11 колонок: sel/Type/Service/PID/Name/SendTime/Filename/Size/Status/Details/id-скрыт, mainKey=id) + пейджер из 4 KPagePushButton + KPageLineEdit. SetPageProvider (DB-фикстуры). Скриншот сверен (поиск + 3 очереди-строки Storage/MPPS/Commitment + пейджер + TR_Rcds:6). ⭐ ВСЕ ТРИ РОДИТЕЛЬСКИХ ВЬЮ (patient/exam/dicom) теперь на полной точности из библиотеки виджетов сессии.
✅ KPatientListWidget + KPatientManagmentUi ПОРТИРОВАНЫ (режим `patientmgmt`, 90-91-й UI-классы) — ⭐⭐ КАПСТОУН СЕССИИ: полный экран управления пациентами, собранный ЦЕЛИКОМ из библиотеки виджетов сессии. KPatientManagmentUi (реф. @0x7a6ea0, base KFullScreenDialog→подставлен KDialog(KDLG_FULLSCREEN), setupUi @0x7a75f8, 1920×1080): ДВА параллельных QStackedWidget. ЛЕВО — grp_opt(280w): nav-меню KPatientListWidget (QListWidget, чёрный стиль, gridSize 215×103, строки TR_Ptnt/TR_Case/TR_DQueue) + HLine + stackedWidget_opt(271, тулбары) + disk-метка(DEVICE-STUB) + btn_exit(TR_Ext(E)). ПРАВО — stackedWidget_tableview(вью). Выбор строки nav → SwitchPage → ОБА стека (ленивые Init*View @0x7a20d8: new KPatientListViewUi+KPatientListOptUi / KExamListViewUi+KExamListOptUi / KDicomQueueViewUi+заглушка — KDicomQueueOptUi не портирован). EntryView/ItemClicked/ExitCurrentView. KPatientListWidget (реф. @0x79e5d8) — тривиальный QListWidget; кастом-item'ы KPatientListWidgetItem упрощены до QListWidgetItem. DEVICE-STUB: disk-поток InitDiskLabel, IPC SubscribeMsg 0x2eeb/0x2ef7/0x2f0b. Порт-хук: PatientView()/ExamView()/DicomView() для инъекции провайдера. Скриншот сверен (title TR_PManagement + nav 3 режима TR_Ptnt-выбран + KPatientListOptUi тулбар 6 кнопок + KPatientListViewUi справа: поиск+таблица 5 строк+пейджер + disk-метка + exit).
✅ KDicomQueueOptUi ПОРТИРОВАН (режим `dicomqueueopt`, 92-й UI-класс): панель действий очереди DICOM (реф. @0x806890, base QWidget). СИБЛИНГ KExamListOptUi/KPatientListOptUi — та же структура (фикс 270×385, QVBoxLayout spacing 13, margins 30/12/19/-1, кнопки 212×46 + спейсер, Tab-циклинг, MoveFocusToFirstWidget, SigToFocusOutCurrentOpt), но ТРИ кнопки: btn_refresh(TR_Rfsh(F1))/btn_resend(TR_Rsnd(F2))/btn_delete(TR_Del(D)). clicked → слоты KDicomQueueViewUi (OnReFreshCurPageData/OnResendSelectedData/OpenDeleteDialog) → 3 своих сигнала. ВПАЯН в KPatientManagmentUi (dicom-страница: заглушка заменена реальным тулбаром). Скриншот сверен (3 кнопки в порядке).
✅ KPatientListEditDlg ПОРТИРОВАН (режим `patienteditdlg`, 93-й UI-класс): форма добавления/редактирования пациента (реф. @0x7cf010, base KDialog, SetKStyle(7)=W1024, setupUi @0x7d03a8, title TR_EPatient, 1024×712). Full-fidelity — реальные KMemComboBox (cmb_patientid/cmb_name/cmb_applicant, tb_QuickInputPatient/Applicant) + KPatientDateEdit (date_dob/date_plantime) + plain QDateEdit (date_applytime min=max=today). grp_patientinfo(3×3): PID/имя/пол(cmb_gender data-int «»3/TR_M 1/TR_F 0/TR_Nknwn 2)/ДР/возраст(валид [1-199])/тел(валид [0-9\-]{1,32})/койка/2 польз-поля. grp_examinfo(2×2): examitem/план-дата(max 2099)/направитель/дата-заявки. Кнопки Exam(TR_Exm)/OK(TR_OK)/Cancel(TR_Ccl) + подсказка TR_RField. КЛЮЧЕВАЯ ЛОГИКА двунаправленный ДР↔возраст: OnChangedPatientDate @0x7ca218 (ДР→GetAge @0x7ca090 в поле, при совпадении возраста рекурсии нет); OnChangedPatientAge @0x7ca138 (введён возраст ≠ GetAge(ДР) → ДР=InvalidDate). DEVICE-STUB: SavePatientInfo/DB/MRU-персист/PublishMsg/exam-workflow/MRU auto-fill (itemSelect→GetAge/GetDoB/GetGender) — заглушки. Скриншот сверен (все поля с верными типами, ДР/план плейсхолдеры, дата-заявки=сегодня, кнопки).
✅ KPatientListSetupDlg ПОРТИРОВАН (режим `patientsetupdlg`, 94-й UI-класс): диалог настройки колонок списка/worklist (реф. @0x7dc730, base KDialog, SetKStyle(7)=W1024, setupUi @0x7de198, title TR_PLSettings). НЕ 57, а 14 чекбоксов в двух группах (реф. абсолютная геометрия → в порте QGridLayout): grb_patientlist (label TR_LDisplay + tip TR_VITAPAPLSWChecked; 9 колонок: chb_useritem1(+edit)/patientid/useritem2(+edit)/applytime/telephone/pregister_number/applicant/bedno/dob) + grb_worklist (label TR_wQuery; 5: worklist_patientid(+edit)/name(+edit)/wregister_number(+edit)/exam_device(+cmb_equipment TR_Escpy)/plantime(+date-range)). Кнопки btn_save(TR_Sve)/btn_exit(TR_Ext); ExitSetupData @0x7dd5a8 спрашивает TR_Sve? (KMessageBox Yes/No). SaveSetupData @0x7dc8e8: isChecked→SetIsShow* обоих хендлеров + SaveConfig + PublishMsg(12004/12008/12007). Без select-all/forced-on. DEVICE-STUB: KPatientListConfigSetupHandler/KWorkListConfigSetupHandler + IPC. Скриншот сверен (2 группы, 14 чекбоксов+paired-инпуты, combo/date-range, Save/Exit). ЗАВЕРШАЕТ семью patient (management/list/edit/setup/opt).
DISCOVERY 2026-07-22: ВСЕ 75 uic-диалогов (Ui_*::setupUi) ПОРТИРОВАНЫ. Остаток UI — hand-coded слой (layout в C++, без setupUi). Кандидаты: KCheckBoxHeaderView (select-all хедер для KTableView), KCalendarWidget (попап дат для KPatientDateEdit-кнопок), KFullScreenDialog (реальная база, подставлял KDialog), KReservedWidget, KThesaurusWidgetUi, KTempletTreeWidget, KImgTableView-кластер (image-review MVC).
✅ KExamListCancelDlg ПОРТИРОВАН (режим `examcanceldlg`, 95-й UI-класс, hand-coded): пикер причины отмены обследования DICOM (реф. @0x7ebea0, base KDialog, SetKStyle(7)=W1024, title TR_CReason). Вопреки имени — НЕ список, а выбор ОДНОЙ причины MPPS (CID 9300). ctor-арг std::string = ключ обследования. QTableWidget table_reson (1 колонка, header TR_CReason, 17 причин GetReasonList @0x7eb370: TR_DCProcedure/EFailure/IPOrdered/…/EChange, коды 110500+row, NoEditTriggers, stretchLastSection) + btn_ok(TR_OK)/btn_cancel(TR_Ccl). SlotClickBtnOk @0x7ec6a8: HaveChosenTableItem<0→KMessageBox warning TR_PSTCReason; иначе CancelSeries+DB+PublishMsg(12006)[DEVICE]+close. Двойной клик = OK. DEVICE-STUB: KDicomInterface::CancelSeries/KExamListDBTableHandler/IPC. Скриншот сверен (17 причин + OK/Ccl).
✅ KCheckBoxHeaderView ПОРТИРОВАН + ВПАЯН в KTableView (режим `tableview`, 96-й UI-класс): заголовок таблицы с чекбоксом «выбрать всё» в колонке 0 (реф. @0x7b2d00, base QHeaderView). Рисует пиксмапами patient/checkbox/checkbox_{checked,unchecked}[_hover].png (НЕ QStyle), центр секции. Три-состояние (m_checkState). mousePress по секции 0 → латч+съесть (подавить сортировку); mouseRelease → тоггл (или partial→Checked)+updateSection+emit SigCheckStausChange(int 0/1). Hover (mouseMove/leave) → *_hover пиксмапы. SetCheckState(all/none/mixed) — вью→заголовок; GetCheckState; UpdateCheckboxHeader. 100% PORT. ВПАЯН: KTableView::InitTableView (при defaultSelCol≥0) ставит его хедером; SigCheckStausChange→чекнуть/снять все строки колонки 0; model SigHeaderCheckBoxStateChange→SetCheckState(any→Partial/none→Unchecked). Скриншот сверен (чекбокс в хедере col0 + строчные чекбоксы). ЗАВЕРШАЕТ чекбокс-систему KTableView (хедер select-all + строки).
✅ KCalendarWidget ПОРТИРОВАН + ВПАЯН в date-поля (режим `calendar`, 97-й UI-класс): кастомный date-picker (реф. @0x7af690). УСТАНОВЛЕНО: это ВЕРБАТИМ-форк исходника Qt5 QCalendarWidget (переименование QCalendarWidget→KCalendarWidget/QCalendarView→KCalendarView, модель стоковая QCalendarModel), поведение стоковое кроме косметики нав-бара (месяц QComboBox/год QSpinBox). Реф.-путь реверса: подкласс стокового QCalendarWidget — идентичные API/сигналы (clicked(QDate)/selectedDate/setSelectedDate/setDateRange). Косметика нав-бара опущена (стоковый эквивалентен). 100% PORT (KSystemSet::GetSystemLanguage локаль + QDate::currentDate — host-safe). ВПАЯН: KPatientDateEdit::PopupCalendar() (public slot) показывает KCalendarWidget Qt::Popup под полем, clicked(QDate)→setDate+close; календарь-кнопки KExamListSearch/KDicomQueueSearch/KPatientListEditDlg (btnStartDate/btnEndDate/btn_dob/btn_plantime) теперь коннектятся к PopupCalendar — ЗАВЕРШАЕТ date-picker flow (кнопки были не подключены). Скриншот сверен (июль 2026, ◀/▶ месяц, weekday Mon–Sun weekend-red, день 22 выбран).
✅ KFullScreenDialog ПОРТИРОВАН + KPatientManagmentUi ПЕРЕБАЗИРОВАН (98-й UI-класс): полноэкранный диалог-база (реф. @0x3e95f0). УСТАНОВЛЕНО реверсом: МНОЖЕСТВЕННОЕ наследование KDialog + KObject. Ctor(parent, id): KDialog(parent,false) + KObject(id,nullptr) + SetKStyle(KDLG_FULLSCREEN). int-арг — НЕ размер, а GLOBAL-ID на in-process шине KObject (InitObject: id∈[0,4999) уникален, -1=K_OBJ_LOCAL не регится); класс существует РАДИ KObject-идентичности (подклассы через неё SubscribeMsg/PublishMsg). Своих членов/хрома/сигналов не добавляет — фуллскрин от KDialog::SetKStyle. 100% PORT. ЗАМЕНИЛ прежнюю подстановку: KPatientManagmentUi теперь `: KFullScreenDialog(parent, 2000)` (реф. id=2000) вместо KDialog+SetKStyle — получил корректную KObject-шину. Регрессия 106/106, шелл рендерится фуллскрин.
✅ KReservedWidget ПОРТИРОВАН (режим `reservedwidget`, 99-й UI-класс): редактор «зарезервированного поля» отчёта (реф. @0x4df450, base QWidget). QVBoxLayout(margins 9): QLabel + вход по enum E_EDITOR_TYPE — реализованы 2 (реф.): E_LINEEDIT(0)→QLineEdit, E_DATEEDIT(1)→QDateEdit(minDate 0001-01-01); прочие → только метка. API: SetLabel/SetLineEditText/GetLineEditText/SetMaxInputLen/SetDate(sentinel→specialValueText « »)/GetDate/GetLineEdit/GetDateEdit/SetIsUsed/GetIsUsed. installEventFilter переопределён — форвардит на активный вход. Сигналов НЕТ (потребитель берёт внутренний виджет). 100% PORT. Скриншот сверен (line «Normal findings» + date «15.08.2026»).
✅ KThesaurusWidgetUi ПОРТИРОВАН (режим `thesauruspick`, 100-й UI-класс 🎉): пикер глоссария/фраз для правки отчёта (реф. @0x4e57b0, base QWidget, 330×647, абс. геометрия). label_glry(20,8,TR_Glry) + tbtn_glry_mng(274,8,«...») + frame(13,38, StyledPanel) → cmb_checkitem(8,11, 5 типов эндоскопа TR_Gspy/Cspy/Bspy/Nlspy/Cholngscpy) + tree_model(8,51, 1 колонка, header скрыт). InitWidget (combo+setCurrentIndex), SwitchEndoScopeType(int, guard≤14→UpdateTreeWidget), SetEndoScopeType, GetTreeWidget, GetThesaurus(item)→QStringList фраз. Вставка pull-based (сигнала НЕТ). DEVICE-STUB: KThesaurusOpt::MakeTree/GetDiseaseContentList (per-scope файлы) → инъектируемые SetTreeProvider/SetPhraseProvider; SlotToOpenThesaurusMngDlg → KThsaurusManageMentUi (портирован). Скриншот сверен (TR_Glry+«...»+combo TR_Gspy+дерево Esophagus/Stomach→фразы). ГРАБЛИ: preview-ключ `thesaurus` коллизировал с self-test → переименован в `thesauruspick`.
✅ KTempletTreeWidget + KTempletTreeWidgetItem ПОРТИРОВАНЫ (режим `templettree`, 101-102-й UI-классы): чекбокс-дерево шаблонов отчёта (реф. @0x560a28, base QTreeWidget+KObject — шина опущена как device). Категории(top-level)→шаблоны(дети), выбор=отметка чекбокса листа. InitLayout @0x55edb0: 4 колонки (col0 stretch title+чекбокс, col1/2/3 fixed 20/30/30), header скрыт, scrollbar always-on, expandsOnDoubleClick off. KTempletTreeWidgetItem : QTreeWidgetItem (payload KReportTemplateItem упрощён до id-строки + заголовок; flags 0x31 Selectable|UserCheckable|Enabled, CheckStateRole col0). ТРИ-СОСТОЯНИЕ: отметка родителя→все дети (updateChildren); смена ребёнка→пересчёт родителя all/none/mixed (updateParent); антирекурсия m_guard. OnItemChanged: col0→три-состояние + emit SigClickItem(id) при отметке ЛИСТА. DEVICE-STUB: данные KReportTemplateManager::GetInstance→GetTemplateLibCfg→KTemplateLibCfg::GetTemplateLib (per-dept шаблоны) → инъектируемый SetTreeProvider. Сигналы реф. SigClickItem/SigAddItem/SigDeleteItem/SigUpdateItem (payload KReportTemplateItem&). Скриншот сверен (Gastroscopy partial-чек + лист ✓ + Colonoscopy, header скрыт, scrollbar).
✅ KImgTableView-КЛАСТЕР ПОРТИРОВАН (режим `imgtable`, 103-106-й UI-классы: KImgTableView+KImgTableModel+KImgTableDelegate+KImgTableItem): грид-таблица тумбнейлов для обзора захваченных изображений (реф. ctor @0x4b9ab0). KImgTableView : QTableView — InitTableView @0x4b9b90 (cellW/H + imgW/H): заголовки скрыты, showGrid off, NoEditTriggers, SingleSelection, WA_Hover, defaultSectionSize=cell. KImgTableModel : QStandardItemModel(rows,cols) — data() @0x4b6798 ТОЛЬКО DecorationRole→QVariant с указателем KImgTableItem* (кастомный метатип, delegate рисует); flat=cols*row+(page-1)*pageSize+col (reshape). KImgTableDelegate : QStyledItemDelegate — paint @0x4b41b0: item из DecorationRole, LoadImgToQImage, тумбнейл по центру (Y чуть выше — место под бейдж), рамка выделения (selected→blue/hover→cyan/иначе lightGray). KImgTableItem : QObject — QImage (ленивая загрузка LoadImgToQImage), type(0=img/1=video), пути, флаги. Подклассы KExamImgView/KUnusedImgView (богатые делегаты: checkbox/order-num/edited-mark, Request*-сигналы, g_eReportEditCurrentPage мульти-выбор) — НЕ портированы (device-heavy overlay/IPC). DEVICE-STUB: источник картинок (exam-dir/unused-pool на диске) + IPC SubscribeMsg(0x2eec) + глобалы → в порте пути тумбнейлов задаёт вызывающий. Скриншот сверен (6 реальных ассетов прошивки в сетке 2×3, тумбнейлы по центру + рамки).
DISCOVERY-2 2026-07-22: OSD-меню-контейнер-семья — след. кластер (KFrame→KOsdMenuBase→KOsdMenu(хостит KOsdMenuCell через setItemWidget)→KOsdSubMenu(хостит KOsdSpin)→KOsdSingleSelectMenu); self-contained: KTimeWasteBar✓/KTextEdit/KImageButton. Коллизии (НЕ трогать): KProgressBar/KWaitProgressBar/KProgressDlg/KControlInfo/KStatisticInfo/KDeviceInfo уже есть.
✅ KTimeWasteBar ПОРТИРОВАН (режим `timewastebar`, 107-й UI-класс): модальный «пожалуйста подождите» с авто-прогрессом (реф. @0x6095b0, base KDialog — НЕ QWidget!). За nSeconds бар едет 0→100% (QTimer 100мс, m_nMax=nSeconds*10 шагов, TimeSlot @0x609538: ++count→pct→≥100 close), resize(480,160), SetKStyle(4)=W480, title TR_Dlg. QGridLayout: topSpacer / label_Text(TR_PWait статичный, max h50, AlignLeft|Bottom) / ProgBar(0..100 «%p%») / bottomSpacer. SetCount/SetEscEnable; keyPressEvent Esc→close только если m_bEscEnable. Своих сигналов НЕТ (close→QDialog::finished). 100% PORT. Скриншот сверен (TR_Dlg+TR_PWait+бар 0%, таймер в grab не тикает).
✅ KFrame + KOsdMenuBase + KOsdMenu ПОРТИРОВАНЫ (режим `osdmenu`, 108-110-й UI-классы): OSD-меню-контейнер, ХОСТИТ портированный KOsdMenuCell (parent-composes-children как patient-shell). KFrame (реф. @0x47f490, base QFrame) — payload-база с ВИРТУАЛЬНЫМ интерфейсом (UpKeyAct/DownKeyAct/Selected/UnSelected/Focused/SetValue/SetDoubleValue/SetStartPoint/IsGreyed/GotFocusIn/UpdateUI — база no-op; конкретные виджеты override). KOsdMenuBase (реф. @0x47a780, base KDialog): m_parentMenu-цепочка (GetOsdMenuLevel walk, root=1), keyPressEvent F12→ExitAllMenus, device-хуки *Impl пустые виртуалы (VideoParamChange/SystemStatusChange/CameraStatusChanged; реф. ctor коннектит KUiMsgProxy/… — опущено). KOsdMenu (реф. @0x479ed0, base KOsdMenuBase): QVBoxLayout(0) + QListWidget(NoFocus, transparent-стиль), безрамочное окно. AddItem(KOsdMenuCell*) @0x479740: SetLocatedMenu+UpdateGreyedFlag+insertItem+setItemWidget+sizeHint. RefreshMenu: старая UnSelect/новая Select + SizeHintRole. UpKeyAct/DownKeyAct (idx±1 wrap), ItemClicked, ConfirmKeyAct (Focus+SetSubWindowPosition+активация virtual). DEVICE-STUB: InitWidget hard-cells (KIrisItem/KRecordItem/…) → вызывающий добавляет через AddItem; KUiMsgProxy nav-сигналы. Скриншот сверен (4 ячейки, DownKeyAct→Record cyan-Select+флаг, Image normal). ЗАВЕРШАЕТ OSD-семью (cell+spin+menu).
✅ KImageButton ПОРТИРОВАН (режим `imagebutton`, 111-й UI-класс): картиночная кнопка на СТИЛЯХ (реф. ctor @0x3e9218 / 4-QString @0x3e9050, base QPushButton). ОТЛИЧИЕ от KImgPushButton/KPagePushButton (те — paintEvent): KImageButton — QSS, UpdateQSS собирает stylesheet с псевдо-состояниями :hover/:pressed/:disabled + setStyleSheet. 4 состояния normal/hover/selected(:pressed)/invalid(:disabled). Strech=1→border-image(масштаб), =0→background-image center. Текст — штатный setText + per-state цвета (#ffffff/#00badb pressed/#4f4f4f disabled), padding TextXPos/YPos. Дефолт 211×46. url=BasePath()+«/»+имя; BasePath=CustomBasePath или «02-data/rw-common/picture». API: SetImage(4)/Set{Normal,Hover,Selected,Invalid}Image/SetCustomBasePath/SetWidth/Height/Size/SetStrech/SetTextX-YPos. Сигнал штатный clicked(). 100% PORT. Скриншот сверен («Menu» border-image+белый текст / «Disabled» invalid-state+#4f4f4f).
✅ KOsdSubMenu ПОРТИРОВАН (режим `osdsubmenu`, 112-й UI-класс): OSD-подменю, хостящее спины (реф. @0x47b4f0, base KOsdMenuBase, (QWidget*, bool bAddReturnBtn)). ХОСТИТ портированные KOsdSpin/KOsdDoubleSpin через AddItem-фабрики от конфигов (реф. строит KFrame-подклассы KOsdSpin02/KOsdDoubleSpin/KOsdStatusLabel; в порте наши QFrame-спины, хостинг обобщён на QWidget*). Шелл: QVBoxLayout(0) + QListWidget(NoFocus, translucent, «color:rgb(160,160,160)», item margin 4px), фикс-ширина 270. AddItem(KOsdSpinConfig)@0x47bd20/AddItem(KOsdDoubleSpinConfig)@0x47be40 строят спин→host. InitWidget @0x47bfe0: height=count*44, клип к 1080, move. Навигация Up/Down/Confirm/клик (курсор выбора). SetValue(row,int/double) — фан-аут в hosted-спин (device→UI). DEVICE-STUB: KUiMsgProxy nav-сигналы (OsdMenuUp/Down/Confirm) + SendToMainCtrl value-push. СИБЛИНГ KOsdSingleSelectMenu (радио + KOsdSingleSelectLabel + ConfirmAction-функторы) — ОТЛОЖЕН (device-callback подсистема). Скриншот сверен (Gain 65/Depth 8/MI 1,2 — KOsdSpin+KOsdDoubleSpin в подменю, SetValue-фан-аут).
✅ KTextEdit ПОРТИРОВАН (режим `ktextedit`, 113-й UI-класс): QTextEdit с тач-перетаскиванием вертикального скроллбара (реф. @0x815c60, base QTextEdit). УСТАНОВЛЕНО реверсом: НЕ инпут-политика (вопреки схожести с KCounterTextEdit/KPasswordLineEdit) — только кастом-скролл для тачскрина. ctor: WA_MouseTracking + ScrollBarAsNeeded + range/step. Члены: m_dragging(+0x30 bool), m_lastPos(+0x34/0x38). eventFilter @0x815d20 (владелец ставит на viewport/scrollbar; в порте самоустанавливаем): press на скроллбаре → выше ползунка PageStepSub / ниже PageStepAdd / на ползунке начать drag (m_dragging=1); move→пересчёт value по дельте (QStyleOptionSlider метрики); release→конец drag. IsOnVerticalScrollBar @0x815cf8 (geometry().contains). Сигналов/инпут-политики НЕТ. 100% PORT (на десктопе нативный скроллбар уже это умеет). Скриншот сверен (12 строк, скроллбар справа).
DISCOVERY-3 2026-07-22: портируемый UI НЕ исчерпан — ещё ~8 (KMsgPopup/KFloatingMsg/KNewTempletEditor✓, KMySlider/KSpinAge/KDateEdit, KIpLineEdit(отложенный single-field IP), KReportPreviewCenterDlg print-preview). KPinyinWidget — heavy-dep (KGooglePinyin IME).
✅ KFloatingMsg + KMsgPopup + KNewTempletEditor ПОРТИРОВАНЫ (режимы `floatingmsg`/`msgpopup`/`newtempleteditor`, 114-116-й UI-классы): KFloatingMsg (реф. @0x68fb48, base QWidget) — плавающий оверлей-список: QVBoxLayout+label_FloatingM(center), m_textList склеен «\r\n», AddText/DelText/SetTextShow/ShowText/ClearTextShow, фон black, SetFloatingRect(move+resize). 100% PORT. KMsgPopup (реф. @0x684788, base QLabel) — авто-тост: внутр. m_pLabel(wordwrap/center) + QTimer(5000мс) автоскрытие, стейт m_status(1 idle/2 showing) с очередью m_pending. Display @0x684a08 (idle→show+timer+status2, showing→queue), HideLabelInfo @0x684e78 (stop→hide/или re-Display pending). UpdatePostion @0x684638 device-позиция над видео → DEVICE-STUB центр/фикс-rect (GetSoftEndoViewConf/getVideoRectForUI). KNewTempletEditor (реф. @0x55cf30, base QTextEdit) — read-only rich-text превью отчёта (ОТЛИЧАЕТСЯ от KTempletTreeWidget=QTreeWidget); ctor setReadOnly(true); контент реф. рендерит KDocumentGenerator(DEVICE)→в порте SetPreviewHtml-стаб. Скриншоты сверены (3-msg overlay / cyan-тост «Image saved» / html-report превью).
✅ KMySlider + KSpinAge + KDateEdit ПОРТИРОВАНЫ (режим `inputwidgets`, 117-119-й UI-классы): три малых инпут-виджета, 100% PORT. KMySlider (реф. @0x78def0, base QSlider): дочерняя QLabel «mySliderLabel»(50×20, белый/transparent/10pt/center) — value с «+» для положительных; SetLabelText @0x78dd98/SetLabelPosition(bool skip)@0x78dce8 (метка к ползунку — реф. латентна, always skip; в порте связал valueChanged→трек)/SetValue; mousePress jump-to-click. KSpinAge (реф. @0x6904b8, base QSpinBox): range 1..199, стартует пустым (clear); CheckText @0x690598 (пусто&&≤0→clear); value() @0x690700 (пусто→0); setValue(0)→clear; keyPress Enter в пустом→clear; eventFilter. KDateEdit (реф. @0x813600, base QDateEdit): ТОТ ЖЕ механизм пустого что KPatientDateEdit (specialValueText + InvalidDate=QDate(2100,1,1)), но плейсхолдер — фикс. бланк-маска «  /  /    » (не формат) + toggle specialValueText⇄«» (date() различает set/unset) + focus-out revert к бланку если не закоммичено. Скриншот сверен (слайдер «+4»@ползунок / age 45 / date бланк-маска + 08/15/2026).
✅ KIpLineEdit ПОРТИРОВАН (режим `iplineedit`, 120-й UI-класс) — РАНЕЕ ОТЛОЖЕННЫЙ single-field IP (реф. @0x6f9030, base QLineEdit; при порте KIpAddrEdit взят composite KIpLineEdit1+KIpAddrEdit, этот устаревший вариант отложен). Весь адрес в одном QLineEdit, БЕЗ inputMask/QValidator — валидация вручную. ctor: WA_InputMethodEnabled=false + textChanged→IpAddressInputMask @0x6faf00 (нормализация formatText + авто-точка при 3 цифрах октета + переписать при изменении, курсор в конец, антирекурсия). formatText @0x6fa530: сплит «.», пер-октетный клип (non-subnet: octet0 1..223, 127/>223 отклон; иначе 0..255; subnet: все 0..255; реф. KMessageBox на выход-за-диапазон → в порте молча клип). text() @0x6f90d8 без пробелов; setText @0x6f9228 («»→скелет « . . . »); isValidIP @0x6f9b38 (4 непустых октета); SetSubnetMaskEnable. 100% PORT. Скриншот сверен (192.168.1.100, text()=«192.168.1.100» valid=yes).
✅ KReportPreviewCenterDlg ПОРТИРОВАН (режим `reportpreviewcenter`, 121-й UI-класс): поверхность превью отчёта (реф. @0x4fc948, base QWidget, Create @0x4fc0c8). QGridLayout + QPrintPreviewWidget(previewPrinter) на тёмном фоне #141519, горизонт. скроллбар скрыт, вертик. кэширован. InitPrinter @0x4fbf18: A4/Portrait/150dpi/PdfFormat (outputFileName examDir — device). paintRequested→OnReportPreview @0x4fdce8 (DEVICE: рендерит report-template модель KReportTemplateManager/KSysReportTempletCfg→KRTSimpleDisplay/KRTTeDisplay→QTextDocument→SplitDocument пагинация header/body/footer + PDF-экспорт; сепаратор-бар #3a95be) → в порте инъектируемый QTextDocument (SetPreviewHtml). Вью-контролы OnBtnFitPage/FitWidth @0x4fbf00/ef8, SetZoomFactor/GetZoomFactor, UpdatePreview @0x4fbec8. Кнопок/тулбара в классе НЕТ (зовёт владелец KReportPreviewDlg — уже портирован). ДОБАВЛЕН Qt5::PrintSupport в CMake (find_package + оба таргета). Скриншот сверен (A4-страница на тёмном фоне: Endoscopy Report + #3a95be-бар + Findings/Diagnosis, вертик. скроллбар). ГРАБЛИ: preview-ключ `reportpreview` занят KReportPreviewDlg → `reportpreviewcenter`.
ИТОГ: ⭐⭐ 121 UI-КЛАСС ЗА СЕССИЮ (…+KDateEdit+KIpLineEdit+KReportPreviewCenterDlg) + 2 апгрейда + 1 перебазировка. ⭐⭐⭐ ПОРТИРУЕМЫЙ UI-СЛОЙ ИСЧЕРПАН: uic-диалоги + custom-widget-библиотека + patient/exam/dicom-подсистема + hand-coded + OSD-меню-семьи + self-contained + message/input/preview-fillers. Остаток — ТОЛЬКО device-bound (live-video/camera-acquisition-hw/KGooglePinyin-IME/KOsdSingleSelectMenu-callbacks).
🔼 ФАЗА АПГРЕЙДА ПОДСТАНОВОК: старые диалоги (портированные ДО библиотеки виджетов) апгрейдим на реальные виджеты — сверяя реф. что за виджет. KDICOMServiceEditDlg АПГРЕЙД: IP-поле (widget_server/IPAddr) QLineEdit+inputMask → РЕАЛЬНЫЙ KIpAddrEdit (реверс подтвердил: единственный кастом-виджет диалога — KIpAddrEdit; cmb_server/serviceType/cmtType/RPD — plain QComboBox; НЕТ KOptionListButton/KLineH/KMemComboBox). Скриншот сверен (4-октетный KIpAddrEdit в TR_SIP). Приём как у KExamListViewUi/KDicomQueueViewUi-апгрейдов. KAddPrinterDlg АПГРЕЙД: device-IP поле (page1 m_pStackedWdt) QLineEdit+inputMask → РЕАЛЬНЫЙ KIpLineEdit (single-field, НЕ KIpAddrEdit!); реверс подтвердил — это ЕДИНСТВЕННЫЙ кастом-виджет диалога: m_pConnectTypeCmb/URL-combo(page0) — plain QComboBox, m_pServiceNameLedit — plain QLineEdit+validator, сепараторов НЕТ (только spacers). НЕ трогал combo/name. Скриншот сверен. KSystemSetDlg АПГРЕЙД: 3 сетевых поля (edt_localip/localnetmask/localgateway) QLineEdit+inputMask → РЕАЛЬНЫЙ KIpLineEdit (edt_macAddress остаётся plain QLineEdit — MAC, не IP); 3 сепаратора секций (line_zoom/color/enhance) QFrame(HLine/Sunken) → РЕАЛЬНЫЙ KLineH (гравированная канавка). Реверс подтвердил: все combo (dateformat/connershape/language/vlsconfig/videoSegmentation/resolution/cornerCutSize) — plain QComboBox (НЕ KOptionListButton/KMemComboBox), НЕ трогал; frame_btn=реальный KParamSetBtn (отложено). Скриншот сверен (KLineH-канавки в TR_Vdeo2/TR_LATime/TR_Ntwrk + KIpLineEdit в TR_LIP/Ntsk/Gtwy). ПАКЕТ АПГРЕЙДА СЕПАРАТОРОВ/ПАРОЛЕЙ (реверс 8 диалогов, субагент, подтвердил по objectName KLineH-ctor shape=6/shadow=0x20 vs plain-QFrame shape=4/shadow=0x30, и KPasswordLineEdit-ctor vs QLineEdit+echo): KLineH-АПГРЕЙД (mkLineH/mkSep-хелпер → new KLineH): KCustomEdit (7 сеп.: line_endoscope*/line_zoom/line_color/line_enhance/line_foot), KGeneralSetDlg (3: frame/frame_2/frame_account_line — «frame»-имена, но реально KLineH!), KScopeInfoEdit (3: line_espec/einfo/uinfo), KStatisticInfo (3: line_1/2/3). KPasswordLineEdit-АПГРЕЙД (пароль-поле lineEdit_passwd → new KPasswordLineEdit, echoMode остаётся ответственностью вызывающего — KPasswordLineEdit сам его НЕ ставит): KLogin, KPUserLoginDlg (тип КОНКРЕТНЫЙ KPasswordLineEdit*, не QLineEdit* — setValidator ПЕРЕОпределён/невиртуален: берёт QRegExp для живой фильтрации). ⚠ ОСТАВЛЕНЫ КАК GENUINE STOCK (реверс подтвердил plain QFrame HLine, НЕ KLineH): KReportEditUi (line_splitor), KReportPreviewDlg (line); контейнер-фреймы frame_page*/frame_hospital* и account-поля lineEdit_id/lineEdit_account — не трогаем. KLineV нигде не используется. 6 диалогов апгрейд, все скриншот-сверены, регрессия 106/106. KPasswordEdit АПГРЕЙД (реверс подтвердил): оба поля password/passwdconfirm = РЕАЛЬНЫЙ KPasswordLineEdit (был QLineEdit); echoMode(Password)+maxLen16 ставит вызывающий (виджет сам НЕ ставит), +setInputMethodHints(0x208), +QRegExpValidator(KAccount::GetPasswordRegExp @0x3f5b48) на ОБА. Регэксп извлечён БАЙТ-ТОЧНО из бинарника (QString::fromAscii(&DAT_00961ba0,0x60) = 96 UTF-8 байт/50 симв): чёрный список fullwidth/CJK-пунктуации `[^\！\『…\。]+$` (вкл. квирк неэкранированного 3-го … в "\…\……" — в char-class инертен); проверено python-декодом (50 симв/96 байт/22 бэкслеша/23 CJK, коды сверены). Тип полей конкретный KPasswordLineEdit* (setValidator невиртуален/переопределён). Скриншот сверен. KPatientManagmentUi сепаратор `line` — реверс подтвердил GENUINE plain QFrame(HLine/Sunken/minW230), НЕ KLineH → оставлен.
⭐⭐⭐ ФАЗА АПГРЕЙДА ПОДСТАНОВОК ЗАКРЫТА: все кастом-виджеты, что подставлялись stock-Qt заглушками (IP-поля→KIpLineEdit/KIpAddrEdit ×3 диалога; сепараторы→KLineH ×4 диалога/16 линий; пароли→KPasswordLineEdit ×3 диалога/4 поля), теперь имеют бинарно-точные реализации, СВЕРЕННЫЕ дизасмом по objectName (не по имени/догадке). Genuine-stock подтверждены и оставлены: KReportEditUi(line_splitor)/KReportPreviewDlg(line)/KPatientManagmentUi(line) — реально plain QFrame; account-поля lineEdit_id/account — реально QLineEdit. Остаток проекта — ТОЛЬКО device-bound (live-video KVideoLabel/KViewHardEndo/KViewSoftEndo/KVideoPlayerOSD/KPipView; camera-acquisition OSD; KGooglePinyin IME; KOsdSingleSelectMenu confirm-callbacks). Регрессия 106/106.
🔼 ПОД-ФАЗА АПГРЕЙДА КАСТОМ-КНОПОК (найдена после закрытия основной — грепом ловится хуже, т.к. заглушка = QPushButton, а не спец-маркер): реф. кастом-кнопки KImageButton/KImgPushButton подставлялись QPushButton. Обе кастом-кнопки УЖЕ портированы. Сайты: KVideoPlayerOSD (8 transport), KExamDetailInfoUi (пагинация), KUnusedImgPlayBar. KVideoPlayerOSD АПГРЕЙД (реверс подтвердил полный спек, base QDialog frameless-fullscreen-оверлей — у нас порт как видимая QWidget-панель): 8 transport-кнопок QPushButton(текст) → РЕАЛЬНЫЙ KImageButton с 4-состоянийными иконками из qss/black/player/btn/*_normal|selected|disable.png (SetImage(n,hover=sel,sel,inv); реф. base "02-data/rw-common/picture" = device-путь, локально ассеты в qss/black/player/btn; base через SetCustomBasePath(theme::root/qss/black/player/btn)). Воспроизведены реф.-квирки: next_frame invalid=next_video_disable.png (баг реф.), start_pause invalid=pause_normal.png (pause_disable.png нет). btn_speed — без иконки, текст "X1.0". Скриншот сверен (все 8 иконок грузятся из прошивки: ◀−/▶+/◀/⏹/▶/X1.0/save/exit). DEVICE (KVideoPlayer движок/seek/speed/2×QTimer/GstDiscoverer) — заглушки. Регрессия 106/106. KExamDetailInfoUi АПГРЕЙД (реверс: base KFullScreenDialog(2004); setupUi создаёт 4× KImgPushButton, InitWidget зовёт InitButtons): 4 пагинации-кнопки (btn_head/btn_pre/btn_next/btn_tail) QPushButton(глифы |< < > >|) → РЕАЛЬНЫЙ KImgPushButton.InitButtons(normal, hover, checked=hover, disable, QSize(48,32), false). База = GetReadOnlyBaseDir + "mainapp/application/qss/icon/pageturning/" (через QDir::absoluteFilePath — C7). Стемы: page_head/page_front(!btn_pre)/page_next/page_tail (_normal/_hover/_disable), ассеты локально в syspreset/mainapp/.../pageturning. Скриншот сверен (все 4 иконки грузятся из прошивки). Левая колонка (btn_report/export/dicom_upload/delete/exit) — реф. подтвердил plain QPushButton, оставлены. Регрессия 106/106. KUnusedImgPlayBar — ПРОПУЩЕН (мёртвый класс, референс есть, но не используется ни одним экраном — только demo в preview_main; апгрейд косметика).
⭐⭐⭐ ПОД-ФАЗА АПГРЕЙДА КАСТОМ-КНОПОК ЗАКРЫТА: KVideoPlayerOSD (8×KImageButton) + KExamDetailInfoUi (4×KImgPushButton) — все с реальными иконками прошивки, сверено дизасмом. Итого фаза апгрейда подстановок (IP/сепараторы/пароли/кнопки) полностью закрыта; портируемый UI-слой и его апгрейд ИСЧЕРПАНЫ. Остаток — device-bound (live-video движок/camera-acquisition/IME/OSD-confirm-callbacks).
🔽 НОВЫЙ ЗАХОД: device-bound-но-портируемые-с-заглушками OSD-классы (как KVideoPlayerOSD доказал — достижимы через DEVICE-STUB-seam). ✅ KOsdSingleSelectMenu ПОРТИРОВАН (режим `osdsingleselect`, реверс: base KOsdSubMenu, тонкий фабричный подкласс). Добавлено: (1) KFrame +3 виртуала (Checked/ConfirmKeyAct(int)/IsSingleSelectLabel — база no-op/false); (2) KOsdSubMenu +InitCheckedItem/RefreshCheckedItem +m_checkedIndex +confirm-flow (Selected→ConfirmKeyAct(seam)→коммит checked, спины KOsdSpin=QFrame cast-промах→не задеты); (3) NEW KOsdSingleSelectLabel:KFrame (QHBoxLayout 9,0,0,0 + label_icon + текст, фикс 250; чек=смена иконки singlechoice_select/normal.png через theme black/osd; IsSingleSelectLabel→true; ConfirmKeyAct→инъект-колбэк); (4) NEW KOsdSingleSelectMenu:KOsdSubMenu (InitConfig(pos, список пар {cfg,action}, checkedIndex) → per-pair KOsdSingleSelectLabel+AddItem+InitWidget+InitCheckedItem, bAddReturnBtn=true). DEVICE-seam: confirm-стратегия ConfirmAction→KUiMsgProxy::SendToMainCtrl(msgType,msgParam,value) заменена инъект-std::function (дефолт no-op). Ассеты локально (singlechoice_*.png). Скриншот сверен (4 строки Auto/Low/Medium/High, Medium отмечен cyan-чеком). Регрессия 106/106 (OSD-семья не задета). ✅ KOsdLabel + KIrisMenu ПОРТИРОВАНЫ (режим `irismenu`, реверс обоих): KOsdLabel:KFrame (size 0x60) — строка-метка text-only, _KOsdLabelConfig={QString text; fn-ptr action} (16 байт, БЕЗ иконки/msg-полей — реверс подтвердил только 2 поля), QHBoxLayout(0, margins 9,0,0,0)+label_title, фикс 250×32; ConfirmKeyAct(key)→action(GetStartPoint,key,GetLocatedMenu). action = реф. Xxx::EnterMenu (открытие под-подменю) — DEVICE-seam, в порте инъект std::function (дефолт no-op, под-подменю не портированы). KFrame расширен: хранимые m_startPoint/m_locatedMenu + Get/Set (SetStartPoint стал конкретным сеттером, был virtual no-op). KOsdSubMenu: +AddItem(KOsdLabelConfig) (строит KOsdLabel — как AddItem(KOsdSpinConfig)); AddItem(QWidget*) ставит SetLocatedMenu(this), InitWidget ставит SetStartPoint(pos) на KFrame-строки. KIrisMenu:KOsdSubMenu(true) — InitConfig(pos): 2 строки TR_OpMode(→KOperationModeSetting::EnterMenu)/TR_IRIS1(→KIrisSetting::EnterMenu, оба no-op в порте) + InitWidget. Скриншот сверен. Форс-мультипликатор: разблокирует семью реактивных OSD-меню (KSnMenu/KButtonDefinitionMenu — те же KOsdLabel-строки). Пропуск: KViewSoftEndo/HardEndo/Base, KVideoLabel (чистое live-video). Отложено: KFeatureMenu/KImageProcessingMenu (VideoParamChanged — нерешённое арх-решение §C). ✅ KSnMenu + KButtonDefinitionMenu ПОРТИРОВАНЫ (режимы `snmenu`/`buttondefmenu`, реверс item-списков) — тривиальные KOsdSubMenu(true)-подклассы по шаблону KIrisMenu: KSnMenu = 2 инфо-строки TR_Mdl:/TR_SN: (action=NULL; значения модель/серийник реф. из GetCamera()->GetCameraInfo() — DEVICE, опущены); KButtonDefinitionMenu = 5 строк TR_PMButton/HDAButton/PAButton/HDBButton/PBButton, единый action=KbuttonSetting::EnterMenu (no-op в порте). NeedToCloseWhenCameraDisconnected()→false = база (реф. переопределяет тем же; не дублируем). KOsdMenuCellGreyedWhenCameraDisconnected НЕ нужен (реверс: не используется этими меню; его юзают KCameraInfoItem/KFeaturesItem/KimageProcesItem). Оба скриншот-сверены (KButtonDefinitionMenu 5 строк, KSnMenu 2). Регрессия 106/106. ✅ KOsdStatusLabel + KPIPViewRect/KPIPView ПОРТИРОВАНЫ (режимы `osdstatuslabel`/`pipview`, реверс обоих). KOsdStatusLabel:KFrame (size 0x70) — строка-статус: QHBoxLayout(0, margins 9,0,9,0) = label_title | spacer | label_status, фикс 250×32; _KOsdStatusLabelConfig={QString title; QString value; int msgType} (реверс: НЕТ action-fn, только msgType); SetStringValue→label_status; ConfirmKeyAct→SendToMainCtrl(msgType) DEVICE-seam no-op. +KOsdSubMenu::AddItem(KOsdStatusLabelConfig). Скриншот сверен (3 строки Brightness/Contrast/Gamma, значения справа). KPIPViewRect:QLabel (size 0x50) — оверлей-маска PiP: НЕ рамка, а КОМПОЗИТ-ВЫРЕЗ (paintEvent CompositionMode_Clear + fillRect/drawRoundRect по m_radiusRect.x/.y — пробивает скруглённую прозрачную дыру); setRect/setRadius/refresh(move+resize)/setVisible. KPIPView:QObject — обёртка над одним KPIPViewRect (реф. 2-й слот +0x18 присваивается ВНЕШНЕ, в бинарнике источник не найден → порт одиночный, честно помечено). БЕЗ device-seam (геометрия от вызывающего). ⚠ ЧЕСТНО: offscreen-превью (grab) НЕ показывает вырез — CompositionMode_Clear без alpha-композитора стирает в непрозрачный чёрный; код построчно сверен с дизасмом (порт верен), скруглённый прозрачный вырез проявляется на device-композиции. Регрессия 106/106.
Семья OSD-меню теперь: KOsdMenu/KOsdSubMenu/KOsdMenuBase/KOsdMenuCell/KOsdSpin/KOsdDoubleSpin/KOsdSingleSelectMenu/KOsdSingleSelectLabel/KOsdLabel/KOsdStatusLabel + меню-фабрики KIrisMenu/KSnMenu/KButtonDefinitionMenu + KVideoPlayerOSD + KPIPView. Строки/ячейки = KFrame-подклассы, seam = action-колбэк/SendToMainCtrl инъект-no-op.
✅ PORT-NOW-БATCH (4 меню, реверс item-списков, режимы imgenhtype/brighteq/denoise/buttonsetting): KImageEnhTypeSetting:KOsdSubMenu(true) — 3 KOsdLabel (TR_StreA/StreB/Ege2, action=EnterMenu no-op), InitCheckedItem(ImagEnhMode()-1 → деф.0). KImageBrightEQSetting/KImageDenoiseSetting:KOsdSingleSelectMenu — числовые "0".."N-1" (N=product.ini [Limit/BrightnessEQ]/[Limit/ImgDenoise], per-model device-config → деф.4), msgType=0x11/0x10, msgParam=индекс, checkedIndex=GetVideoParam()->GetBrightEQConfig()/GetDenoiseConfig() (device→деф.0). KbuttonSetting:KOsdSubMenu(true) — 12 СТАТИЧЕСКИХ single-select-строк (TR_Frz/Zm1/IRIS1/AGC1/IEnh/Snp/Brtnss±/Ctrst/WBalance/LMode/Rcd, funcId 0..11; гейты IsFuncEnbale id1/10/11 → в порте true), msgType=0x2a, msgParam=keyIndex; checkedIndex через FunctionIdToIndex(GetButtonFunctionId(...)) — GetButtonFunctionId=user-config device → деф.funcId0→index0; UpdateCheckedItem(a,b)→InitCheckedItem(b); UserSetChange-сигнал опущен. +KOsdSubMenu::AddItem(KOsdSingleSelectLabelConfig) (single-select в не-single-select-меню). Все device-читатели (video-param checked, product.ini счётчики, button-assignment, feature-флаги) заглушены документированными дефолтами. Скриншоты сверены (brighteq "0"-"3" checked 0; buttonsetting 12 строк checked TR_Frz). Регрессия 106/106. OSD-меню-ПОДКЛАССЫ: 7/17 портировано (KButtonDefinitionMenu/KIrisMenu/KSnMenu + KImageEnhTypeSetting/BrightEQ/Denoise/KbuttonSetting); остаток — DEFERRED-10 (переопределяют VideoParamChangeActImpl/SystemStatusChangeActImpl, ждут video-proxy-решения) + корневое KOsdMenu::InitWidget со спец-ячейками (отдельный заход).
✅ КОРНЕВОЕ OSD-МЕНЮ ПОРТИРОВАНО (капстоун семьи, режим `osdrootmenu`, реверс KOsdMenu::InitWidget @0x479c70 + 7 спец-ячеек). KOsdMenu::InitWidget собирает фикс-порядок ячеек: KIrisItem(TR_Mode→KIrisMenu)/KimageProcesItem(TR_IParameters→KImageProcessingMenu-deferred)/KExitItem(TR_Ext→close)/KFeaturesItem(TR_Fnctn→KFeatureMenu-deferred)/KButtonDefineItem(TR_Button→KButtonDefinitionMenu)/KCameraInfoItem(TR_Camera→KSnMenu); KRecordItem(#2) DEFERRED (device: system-status/USB/record; реф. гейт IsVideoRecordEnable). Новые (KOsdRootMenuItems.h/.cpp): KOsdMenuCellGreyedWhenCameraDisconnected (тонкая KOsdMenuCell-база, camera-grey seam стаб) + 6 ячеек. Каждая ячейка = SetIcons(реальные иконки прошивки qss/black/osd: iris/imageprocessing/exit/features/«button definition»(с пробелом!)/equipment) + SetTitle(tr) + ConfirmAct override (открытие подменю/close). KOsdMenuCell +virtual ConfirmAct() + SubWindowPosition(); KOsdMenu::ConfirmKeyAct зовёт cell->ConfirmAct(). Конфирмы к портированным подменю (Iris/Sn/ButtonDefinition) реальные; к deferred (ImageProcessing/Feature) — no-op. Скриншот сверен (TR_Mode iris-иконка selected cyan + TR_IParameters sliders). Регрессия 106/106.
✅ ПЕРЕКЛАССИФИКАЦИЯ DEFERRED→PORT-NOW: «DEFERRED» из-за VideoParamChangeActImpl-оверрайда НЕ блокирует UI-порт (хук = device re-sync, в порте no-op = база; списки статические/стаб-дефолт). Портированы 3 (режимы contrast/zoom/opmode, реверс подтвердил статические списки): KContrastSetting:KOsdSingleSelectMenu — 3 строки TR_Low/Mdle/Hgh (msgType=0xe, ранг 0/1/2); KZoomSetting:KOsdSingleSelectMenu — 3 ЛИТЕРАЛА L1/L2/L3 (НЕ tr, msgType=8); KOperationModeSetting:KOsdSubMenu(true) — 10 tr-строк (TR_LMode1/TMode/HMode/Cmode/AMode/SMode/TMode1/Umode/Rmode1/FMode, msgParam=channel, msgType=0x1d, выбор=индекс). Все checkedIndex=GetVideoParam()->[оффсет] (device→деф.0); VideoParamChangeActImpl (a==0x17/6/0x18→InitCheckedItem) — no-op в порте. + WIRED KIrisMenu TR_OpMode → KOperationModeSetting (реальное открытие под-подменю). Скриншоты сверены (zoom L1 checked; opmode 10 строк TR_LMode1 checked). Регрессия 106/106. OSD-ПОДКЛАССЫ: 10/17. ✅ +2 (режимы imgenh/irisset): KImageEnhSetting:KOsdSubMenu(true) — 3 ЛИТЕРАЛА L1/L2/L3 (msgType=0xc, msgParam=channel; AllowedToOpen=IsImageEnhEnable/ctor-side SetImageEnhMode — device, опущено); KIrisSetting:KOsdSubMenu(true) — 3 tr TR_FScreen/WScreen/CScreen (msgType=0x1c) + static EnterMenu-фабрика. WIRED KIrisMenu TR_IRIS1 → KIrisSetting::EnterMenu (обе строки KIrisMenu теперь реальны). Скриншоты сверены. Регрессия 106/106. OSD-ПОДКЛАССЫ: 12/17. Остаток-5. ✅ +4 (режимы colorset/lightlevel/featuremenu/imgprocmenu, реверс): KImageColorSetting:KOsdSubMenu(true) — 3 KOsdSpin R/B/C (-15..15 шаг 1, def=0(device), msgId=0/1/2 ctxId=26); KLightLevelSetting:KOsdSubMenu(true) — 1 KOsdSpin L (1..19, def=1(device), ctxId=31); KFeatureMenu:KOsdSubMenu(true) — 7 KOsdStatusLabel (TR_AGC1/Frz/Moire/WBalance/SRemoval/HDR/Tmr, значения ""=device-UpdateStatus, msgType 7/9/0x1e/0xd/0x12/0x13/0x1025); KImageProcessingMenu:KOsdSubMenu(true) — ХАБ 8 KOsdLabel (БЕЗ сепараторов — реверс опроверг: все KOsdLabel: TR_Brtnss1→KLightLevelSetting/TR_Zm1→KZoomSetting/TR_Enh→KImageEnhTypeSetting/TR_Ctrst→KContrastSetting/TR_Col→KImageColorSetting/TR_LMode→KVlsModeSetting(deferred nullptr)/TR_IDenoise→KImageDenoiseSetting/TR_BEquilibria→KImageBrightEQSetting — все action-лямбды к портированным). Каждый с static EnterMenu-фабрикой. WIRED корневые KimageProcesItem→KImageProcessingMenu, KFeaturesItem→KFeatureMenu (корневое меню теперь полностью навигабельно, кроме Record). Все spin-диапазоны из immediates (не гаданы), device-хуки no-op, def/значения = device→стаб. Коллизия ключа imgproc→imgprocmenu. Скриншоты сверены (colorset R/B/C спины; imgprocmenu 8 строк). Регрессия 106/106. OSD-ПОДКЛАССЫ: 16/17 (остаток: KVlsModeSetting — config-driven из coldlight.ini, нужен per-product путь).
⭐⭐⭐ СЕМЬЯ OSD-МЕНЮ ЗАВЕРШЕНА 17/17 (режим vlsmode): KVlsModeSetting:KOsdSingleSelectMenu — список КОМБО спектральных режимов из РЕАЛЬНОГО coldlight.ini (KColdLightConfig::GetVLSConfigDisplayList, путь SystemPath/coldlight/X-2600/X-2600B/coldlight.ini — как self-test coldlight; НЕ стаб!). Каждая строка = combo.join("->") (WL->SFI->VIST/EWL->SFI->VIST/SFI->VIST/EWL->SFI — реальные данные прошивки), msgType=6, msgParam=индекс. FindTheItemIndex — pure-скан (портирован as-is). checkedIndex=GetSystemStatus VLS_MODE (device→деф.0). SystemStatusChangeActImpl no-op. WIRED KImageProcessingMenu TR_LMode → KVlsModeSetting. Скриншот сверен (4 реальных комбо, первое checked). Регрессия 106/106.
ИТОГ OSD-СЕМЬИ: 17/17 подменю + корневое KOsdMenu + item-виджеты (KOsdLabel/KOsdStatusLabel/KOsdSingleSelectLabel/KOsdMenuCell/KOsdSpin/KOsdDoubleSpin) + 6 корневых спец-ячеек + KVideoPlayerOSD + KPIPView. Полностью навигабельно (все ConfirmAct/EnterMenu wired, кроме device-only KRecordItem). Seam-паттерн: action-колбэки/SendToMainCtrl/VideoParam-хуки — инъект-no-op; списки статические или из config-файлов (coldlight.ini/product.ini). ⭐ ПОДСИСТЕМА ЭКРАННОГО МЕНЮ ИСЧЕРПАНА. 
🔷 НОВЫЙ КЛАСТЕР (triage-реверс: 3 портируемых UI-кластера — annotation-editor/pinyin-IME/table-MVC). ✅ КЛАСТЕР 1 «АННОТАТОР ИЗОБРАЖЕНИЯ» (режим `imgcanvas`, реверс): KImageEditorGraphicsView:QGraphicsView — НЕ рисование пером, а РАССТАНОВКА фикс. курсор-пиксмапов (exam_detail::E_CURSOR_TYPE: Lock=0/ArrowRightDown=1/RightUp=2/LeftUp=3/LeftDown=4/Point=5) кликом по фону; клик в drawableRect при type!=Lock → AddCursorPixmapItem+сигнал DrawEventOccurs; per-type hotspot-офсет {1:(13,13),2:(0,13),3:(0,0),4:(13,0),5:(0,0)}; SetCursorType/UndoLastItem/RemoveAll/GetDrawHistory/HasDrawn. Device-иконки стрелок (GetReadOnlyBaseDir) → глиф-замена (програмный рисунок cyan-стрелок/точки). KImageEditorSerialization (singleton): структуры KCursorInfo{int a,b; float x,y; int type}/KImgEditInfo{4int + 2 вектора меток(img/body) + name} + Save/LoadThumbConfigFile; реф. Boost text_oarchive → в порте QDataStream (тот же порядок полей, самодостаточно). Мост вью↔данные — в хосте (масштаб orig/bg). АПГРЕЙД KImageEditor: placeholder QGraphicsView → реальный KImageEditorGraphicsView + 5 радио-инструментов wired на SetCursorType. Скриншот сверен (4 метки ↘/↙/•/↖ на фоне). Регрессия 106/106. Остаток кластеров: 2 (pinyin-IME family, table-MVC family). 
✅ КЛАСТЕР 2 «PINYIN IME» — KPinyinWidget ПОРТИРОВАН (режим `pinyin`, реверс): базовый попап-кандидатов пиньинь (реф. KPinyinWidget : QFrame singleton, size 0xd8). Безрамочный always-on-top translucent-попап: строка-спеллинг (QLineEdit) + ряд кандидатов (реф. QListWidget IconMode → в порте ряд QLabel для надёжного рендера) + «<»/«>» страницы. eventFilter на целевом edit: a-z→AppendPinyin, 1-7→выбор, Space→подсвеченный, -/=/стрелки→страницы/курсор, Enter→сырой спеллинг, Backspace/FocusOut. Выбор коммитится в edit через QInputMethodEvent::setCommitString (реф. точно). 7 кандидатов/страница; UpdateCandUI «1.你». ENGINE seam KGooglePinyin::search/get_candidate → ЗАГЛУШКА (демо-словарь ni→你尼泥呢拟逆腻/hao/wo). Скриншот сверен (ni + 7 ханьцзы, 1.你 подсвечен). Регрессия 106/106. Остаток: InputMethod/KSmallLangInputMethod-роутинг + KGooglePinyin-движок (тяжёлый) + КЛАСТЕР 3 table-MVC. 
✅ КЛАСТЕР 3 «TABLE-MVC» ШАБЛОН — KExamImg-триплет ПОРТИРОВАН (режим `examimg`, реверс): KExamImgModel:KImgTableModel / KExamImgView:KImgTableView / KExamImgDelegate:KImgTableDelegate (базы уже были). Сетка 1×10 (горизонт. полоса), ячейка 159×130, тумбнейл 130×110, скругление r8. data(DecorationRole) отдаёт KImgTableItem*; делегат рисует ВСЁ: центрир. скруглённый тумбнейл + order-num-бейдж (top-right 30×20, darkGray, белый Arial-12 «N+1») + edited-mark + рамку состояния (blue выбран/cyan hover/lightGray, width 2 — все константы из .data). Расширен KImgTableItem: +OrderNum/+Edited/+SetImage(стаб-тумбнейл без диска). Источник строк реф. = глобальный g_vecSelectedItems (device) → LoadImages-seam (инъект-список). Вью: клик/двойной-клик → сигналы RequestSwitchImgOfAddMark/RequestUpdateUnusedImg. Скриншот сверен (8 цветных тумбнейлов, бейджи 1-8, edited-точки на #2/#5). Регрессия 106/106. Шаблон для остальных триплетов (KExamDetail/KUnusedImg/KDICOMSrvList — почти-клоны). 
✅ +2 триплета (режимы unusedimg/examdetail, реверс-дельты): KUnusedImg = KExamImg + filename под тумбнейлом (сетка 3×7; одиночный клик инвертирует выбор + сигнал RequestUpdateSelectedImgFromUnusedView; источник = скан exam-dir + QStringList pre-selection → g_vecSelectedItems). KExamDetail = делегат СВОПает order-num-бейдж → ЧЕКБОКС (top-right, галочка cyan при выборе) + filename + type-aware (0=image/1=video/2=report TR_Rprt); сетка динамическая (в порте ctor rows×cols); двойной клик инвертирует выбор + SelectedChanged + type-диспетч (в порте сигнал). Расширен KImgTableItem +FileName. Рамка/тумбнейл идентичны KExamImg. Скриншоты сверены (examdetail 3×5 с чекбоксами+filename; unusedimg 3×7 с бейджами+filename+edited-точки). Регрессия 106/106. Cluster 3: 3/4 триплета (остаток KDICOMSrvList). 
⭐ КЛАСТЕР 3 ЗАВЕРШЁН 4/4 — KDICOMSrvList-триплет ПОРТИРОВАН (режим `dicomsrvlist`, реверс): текст-таблица DICOM-сервисов (НЕ грид тумбнейлов). KDICOMSrvListModel:QAbstractTableModel — 6 колонок (заголовки TR_Susing/SType/SName/ATitle/SIDName/Prt); data(DisplayRole) по колонкам: [0]«» (чекбокс), [1] тип tr(TR_Strge/SCommitment/Wrklst/MPPS), [2] Name, [3] AE Title, [4] SID, [5] Порт; TextAlignment=center; UserRole→enabled. KDICOMSrvListDelegate — колонка 0 рисует РЕАЛЬНЫЙ checkbox_checked/unchecked.png (qss/black/icon/checkbox/new), 1-5 обычный текст. KDICOMSrvListView — колонки stretch, vheader скрыт, SelectRows/SingleSelection/NoEdit; клик по колонке 0 инвертирует enabled. Источник строк реф. = KDICOMConf (device, 4 суб-списка Storage/Worklist/MPPS/Commit) → в порте setRows-стаб. Скриншот сверен (6 колонок, 4 сервера, чекбоксы вкл/выкл). Регрессия 106/106.
⭐⭐⭐ CLUSTER 3 (TABLE-MVC) ИСЧЕРПАН: 4 триплета (KExamImg/KUnusedImg/KExamDetail/KDICOMSrvList). Итого 3 портируемых UI-кластера (annotation-editor/pinyin-IME/table-MVC) — KDICOMSrvList закрывает последний. Остаток: Cluster 2 хвост (InputMethod-роутинг + KGooglePinyin-движок). 
🔷 РЕЗИДУАЛ-ВИДЖЕТЫ (discovery-триаж): ✅ KEmpDateEdit ПОРТИРОВАН (режим `empdateedit`, реверс): nullable дата-редактор — тонкий QDateEdit-подкласс с ПУСТЫМ состоянием (реф. бланчит внутренний lineEdit; в порте — specialValueText+minimumDate как sibling KDateEdit, надёжный рендер). InvalidDate=QDate(2100,1,1). setDate/setTime/setDateTime: валид→un-empty, sentinel+canEmpty→empty. focusIn заполняет дефолт-датой+подсветка секции; focusOut без правки→снова пусто (m_guard). LineEditSectionAt (3 раскладки формата), SetEmptyAble/clear/date()→null-при-пустом/validate→Acceptable/sizeHint+3. Всё редактирование/клавиши/клип — штатные QDateEdit (реф. НЕ переопределяет). Скриншот сверен (пустое «----/--/--» + заполненное 2026-07-22). Регрессия 106/106. Остаток резидуалов: KFlexEndoBtnGuide(нет ассета)/KOsdSpin02/KOsdReturnLabel/KPatientView + Cluster2-хвост InputMethod-роутинг. 
✅ KPatientView ПОРТИРОВАН (режим `patientview`, реверс): форма инфо о пациенте (реф. QWidget+KObject, size 0x1f8; KObject-шина опущена). 9 полей с иконками (qss/black/icon/patient/*.png) в порядке: Name(KMemComboBox)/Gender(QComboBox 4: blank/TR_M=1/TR_F=0/TR_Nknwn=2)/Age(KSpinAge)/DoB(KEmpDateEdit)/Applicant/PatientID(KMemComboBox)/ExamNo(ro QLineEdit)/Custom1/Custom2. Doctor(bit0x10) скрыт. Реф. НЕ QLayout (абс. стек шаг 49px, фикс-ширина 178) — в порте QVBoxLayout из строк [иконка 28×28 | редактор 178]. DEVICE-seam: реф. LoadPatientInfo/SavePatientData через KExamBussinessHandler(DB)+KObject-шина → в порте setPatient(KPatientInfo)/GetPatient()+сигнал; KMemComboBox DB-автодоп no-op. Маппинг полей §5 (gender findData, DoB parse yyyy-MM-dd, age clamp; examNo/doctor read-only не пишутся). ClearWidgetInfo. Скриншот сверен (John Smith/TR_M/47/1979-03-15/Dr.House/PID-004821/EX-.../Ward 3B/Referral, все иконки). Регрессия 106/106. Использует свежий KEmpDateEdit (DoB). Остаток: Cluster2-хвост InputMethod-роутинг, QuickInput-MVC, RT-render-tail, KFlexEndoBtnGuide(нет ассета). 
⭐ CLUSTER 2 ЗАВЕРШЁН — InputMethod РОУТЕР ПОРТИРОВАН (режим `inputmethod`, реверс @0x761568): singleton-маршрутизатор ввода по языкам. _KLanguageType {0=Chinese/1=English/2,3,6=Latin/4=French/5=Russian/7=Polish}. SetFunctionMap биндит 2/3/6→InputLatin, 4→French, 5→Russian, 7→Polish (Chinese=0 НЕ регистрируется — дефолт-ветка). InputMutiLang(QKeyEvent*): focusWidget + ImEnabled/ImHints-гейт (0x30388 отсев паролей); English→native, Chinese→InputChinese, иначе map-lookup. InputChinese: CheckGoogle→keypress→без Ctrl→буква a-z → KPinyinWidget.SetEditWidget+AppendPinyin+show; не-буква→QueryPunctuation. Latin/Russian/French/Polish→CommitText (упрощ.: печатный текст→QInputMethodEvent commit; реф. через KSmallLangInputMethod). SEAM: активный язык реф. languageConfig/KSystemSet::GetInputMethod(QSettings) → в порте SetActiveLanguage (дефолт Chinese). Реф. вызов — QApplication::notify override (KAppThread::notify) на каждый keypress. +KPinyinWidget CheckGoogle()/QueryPunctuation() (стабы). ⚠ ЧЕСТНО: offscreen-превью НЕ даёт QApplication::focusWidget() → интегрированный focusWidget-путь не рендерится; роутер сверен ДИЗАСМОМ+сборкой, а цель (KPinyinWidget) показана накормленной ТЕМ ЖЕ путём InputChinese (SetEditWidget+AppendPinyin «ni»→7 ханьцзы). Регрессия 106/106.
ИТОГО 3 discovery-кластера: Cluster1(annotation-editor)✓ Cluster2(pinyin-IME: KPinyinWidget+InputMethod)✓ Cluster3(table-MVC ×4)✓. Резидуалы: KEmpDateEdit✓/KPatientView✓. 
🔷 RT-RENDER-TAIL — ФУНДАМЕНТ ПОРТИРОВАН (self-test `rtdatasource`, регрессия 107/107): KRTAbsDataSource — АБСТРАКТНЫЙ инъектируемый источник данных report-template движка (реф. base @0x5892a0, vtable @0xa210c8, 12 слотов — порядок воспроизведён точно: dtor×2, UpdateDataSource, GetTextData, GetTextOptional, GetTextGroupData, GetImageData, GetImageGroupData, GetSubData, GetMixData, GetMix2Data, CacheData). Реф. 6 core-Get* = assert(false) → в порте ЧИСТО-ВИРТУАЛЬНЫЕ (форсируют override); GetTextOptional/GetMix2Data → false; UpdateDataSource/CacheData → no-op. SplitDataSrcID — НЕвиртуальный хелпер через портированный report_template::RevertPathByID(id,","). Семантика Get*(id,out&): заполнить out + true, если id разрешён. + KRTDataSourceStub — канистра (SetText/Image/Sub/TextGroup/ImageGroup/Mix + Get* из карт). Self-test: round-trip text/image/sub, unknown→false, Split("src,key")→(src,key), без разделителя→false, Optional→false. Это ОДИН seam «Simple»-движка (KRTSimpleDisplay/CreatorContext — следующий слой: реф. Display итерирует m_lstItems→CreateItem→creator-реестр из 9 элемент-типов; KReportDisplayParam layout не декомпилирован полностью). COVERAGE.md регенерён: 181 кл/1842 мет (26.3%), было 12.8%. + полный экран patient-management (все 3 страницы patient/exam/dicom с реальными тулбарами) собран из библиотеки виджетов. — все кастом-виджеты, что подставлялись stock-Qt заглушками в ~60 диалогах, теперь имеют бинарно-точные реализации. Все скриншот-сверены, регрессия 106/106 всю сессию.
СЛЕДУЮЩИЙ ФОКУС: реверс встраиваемых вью/оверлеев НА ПОЛНОЙ ТОЧНОСТИ (используя реальные виджеты, без подстановок). Discovery-реверс 2026-07-22 нашёл семью patient/exam-list (~10 портируемых): KMessageFrame✓, KExamListOptUi, KPatientListOptUi, KExamListSearch, KDicomQueueSearch, KLayOut, KPatientListViewUi(large), ReportConfigDlg(large 278w), KPatientListSetupDlg(57 chk), KPatientDateEdit/KPageLineEdit/KFUpbar/KFBackground(tiny prereqs). ⚠ KVideoCal=C6-коллизия (skip), KPasswordEdit уже портирован (skip).
✅ KMessageFrame ПОРТИРОВАН (режим `messageframe`, 81-й UI-класс): динамический 5-строчный лог сообщений (реф. @0x682ab0, base QFrame, setupUi @0x683750). QFrame 370×200 + QVBoxLayout «verticalLayout» + 5 QLabel (label_message1..5, sizePolicy Fixed/Expanding, minimumSize(360,0), пустой текст). Состояние QList<_KShowMessage{QString text; int count}> + m_startIndex. AddShowMessage @0x683020 (дедуп по тексту + append + автоскролл к новейшим), ClearShowMessage @0x683630, UpdateShowMessage @0x682be0 (с startIndex, split '\n', drop empty, префикс «-», разложить по 5 меткам), ShowMessageCheck @0x683228 (countdown каждого count, удалить 0, скролл/сброс). Сигналов/device НЕТ — владелец гонит ShowMessageCheck по таймеру + подаёт AddShowMessage. 100% PORT. Скриншот сверен (4 строки с «-», многострочное split, дедуп «Probe connected»).
✅ KExamListOptUi ПОРТИРОВАН (режим `examlistopt`, 82-й UI-класс): панель действий списка осмотров (реф. @0x7f1630, base QWidget, setupUi @0x7f1b20). Фикс 270×385, QVBoxLayout (spacing 13, margins 30/12/19/9), 6 кнопок 212×46 + нижний вертик. спейсер, в порядке: btn_report(TR_Rprt(F1)) / btn_export(TR_Expt(F2)) / btn_upload(TR_DUpload(F6)) / btn_cancle_check(TR_CExam(F7)) / btn_delete(TR_Del(D)) / btn_set(TR_Set(F8)). Реф. ctor берёт KExamListViewUi* как back-pointer, clicked каждой → слот view (OpenReportEdit/ReadyToExportCheckedItem/ReadyToUploadCheckedItem/OpenCancelExamDialog/ReadyToDeleteCheckedItem/OpenSetupDialog) — VIEW-seam ЗАМЕНЁН на 6 своих сигналов (sigReport/…/sigSetup). Собственный реф.-сигнал SigToFocusOutCurrentOpt (Tab с последней кнопки), MoveFocusToFirstWidget, Tab-циклинг через eventFilter, setTabOrder-цепочка. 100% PORT (VIEW-действия — seam). Скриншот сверен (6 кнопок в порядке, тексты+хоткеи).
✅ KPatientListOptUi ПОРТИРОВАН (режим `patientlistopt`, 83-й UI-класс): СИБЛИНГ KExamListOptUi (реф. @0x7d2a78, setupUi @0x7d2da0). Та же структура (QWidget фикс 270×385, QVBoxLayout spacing 13, 6 кнопок 212×46 + спейсер, Tab-циклинг, MoveFocusToFirstWidget, SigToFocusOutCurrentOpt), но margins 30/12/19/-1 и ДРУГИЕ кнопки: btn_check(TR_Exm(F1)) / btn_add(TR_Add(F2)) / btn_edit(TR_Edt(F6)) / btn_download(TR_Dwnld(F7)) / btn_delete(TR_Del(D)) / btn_set(TR_Set(F8)). clicked → слоты KPatientListViewUi (OnTriggerExam/OpenAddDialog/OpenEditDialog/OnDownloadWorklist/OpenDeleteDialog/OpenSetupDialog) ЗАМЕНЕНЫ на 6 своих сигналов. Скриншот сверен.
✅ KPatientDateEdit ПОРТИРОВАН (режим `patientdateedit`, 84-й UI-класс): дата-эдит с пустым/placeholder-состоянием (реф. @0x7b7e88, base QDateEdit). Ранее подставлялся голым QDateEdit. Пустое = specialValueText==m_format (upper-cased формат как плейсхолдер, деф. «MM/DD/YYYY»), дата на minimumDate. Сентинел date() при пустом — InvalidDate=QDate(2100,1,1). setDate @0x7b85e0 (d==Invalid→плейсхолдер, иначе specialText=""+setDate+dateChanged), setDisplayFormat @0x7b8018 (m_format=f.toUpper(), при пустом переприменить — emptiness проверять ДО смены m_format!), clear/SetPlaceholderText/SetTextMargins. Цвет: серый #999 плейсхолдер / тёмный #3D3D3D значение (toggle на textChanged). focusInEvent: клик по пустому авто-сеет QDate::currentDate() (системные часы, не device). keyPressEvent: цифра в пустое сеет сегодня, Backspace/Delete на полном выделении→pendingClear→focusOut очистит. 100% PORT. Скриншот сверен (пустой YYYY-MM-DD серый + дата 2026-07-22 тёмная). ГРАБЛИ: (1) specialValueText показывается только когда дата==minimumDate — в ctor выставить setDate(minimumDate()); (2) фокус авто-сеет сегодня — для скрина пустого нужен focus-sink. ВЕХА: 50 UI-классов за сессию, все скриншот-сверены, регрессия 106/106 всю сессию. ИТОГ UI-фазы: 40 UI-классов за сессию (35 диалогов над KDialog разных стилей W320/W460/W700/W1024/FULLSCREEN + 5 встраиваемых QWidget/QFrame-вью), все скриншот-сверены, регрессия 106/106 всю сессию; пойманы коллизии имён (KVideoCal C6, KFactoryOptions C5) и preview-ключей (reportedit→reporteditui, coldlight→coldlightadj). УРОК: имя из бинарника + грепать до порта. Иконка btn_close — реф. close.png (device asset), у нас текст ✕. Существующие
...
templetmodel, dimmer, printdata, legalnotice, flexbtn, osdspin02, imglistcell, patnavitem, evtfilters, recorditem, poweroff, opendlgs, autotest, exambiz, exportrec, imgproc, reportedit, videoplayer, smalllang, dimming, endoscope, camera, lcdproxy, factoryopt, docgen, monitor, rtcreator, videoipc, timemng, selftest, autotestthread, mcuuart.
