...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 64  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **64 self-test-режима** (все PASS, регрессия —
...
затронуто 72 класса / 723 метода (11.2%)**. Это нижняя оценка (считает совпадение имён;
...
**ПОСЛЕДНЕЕ (эта сессия): `KTableBlock`** (`app/src/report/`, self-test `tableblock`) — модель
