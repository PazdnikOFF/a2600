...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 74  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **74 self-test-режима** (все PASS, регрессия —
...
затронуто 82 класса / 831 метод (12.9%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KSysReportTempletCfg` — ЗАПИСЬ каталога + мутаторы KTempletBaseInfo
