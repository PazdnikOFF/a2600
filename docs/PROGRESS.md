...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 72  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **72 self-test-режима** (все PASS, регрессия —
...
**ПОСЛЕДНЕЕ (эта сессия): `yxyDES2` — DES (self-test `des`, `app/src/kernel/`)** — РАЗБЛОКИРУЕТ
