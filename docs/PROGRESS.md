...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 69  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **69 self-test-режимов** (все PASS, регрессия —
...
затронуто 78 классов / 772 метода (12.0%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `XmlParser` (self-test `xmlparser`, `app/src/report/`)** — тонкая
