...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **79 self-test-режимов** (все PASS, регрессия —
...
**ПОСЛЕДНЕЕ (эта сессия): парсер скриптов автотеста (self-test `autotest`,
...
templetmodel, dimmer, printdata, legalnotice, autotest.
