...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 62  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **62 self-test-режима** (все PASS, регрессия —
...
затронуто 70 классов / 718 методов (11.2%)**. Это нижняя оценка (считает совпадение имён;
...
**СЛЕДУЮЩИЙ ШАГ — ГОТОВ К РЕАЛИЗАЦИИ (декод сделан, кода нет): `KImageBlock`** — прямой
сиблинг KTextBlock (модель RT_IMAGE_BLOCK, НЕ виджет — нет vtable, off-device). Класть в
`app/src/report/KImageBlock.h/.cpp` по образцу KTextBlock, self-test `imageblock`.
Поля (sizeof 0x98, БЕЗ m_bHideKey): KReportTemplateItem* @0x00, KReportTemplateDataNew* @0x08,
KReportTemplateItemConfig m_itemConfig @0x10, map<string,string> m_mapTextParam @0x68.
Методы (11): ctor(item,dataNew) [assert non-null + InitItemConfig]; InitItemConfig
(идентичен KTextBlock: m_itemConfig = dataNew->m_mapItemConfigs[item->m_strID]);
ElementId()=QString(item->m_strID); **ImageName()=QObject::tr(item->m_strName)** (tr от Name!);
**Url(bool& valid)** const; **Width()**=ConvertStringToInt(itemcfg.attr "ImageWidth"), **-1**
если нет/пусто; **Heigth()** (sic) то же с "ImageHeight", -1; **GetAlign()**=itemcfg.attr
"AlignH" (как есть, "" если нет); GetTemplateItem()=m_pTemplateItem; GetItemConfig()=копия;
SetTextParam(map). Width/Heigth/GetAlign читают ITEM-CONFIG-атрибуты (аналог KTextBlock::attr,
НЕ стиль). Url-логика: str=item->m_strDataSrc; если m_mapTextParam непуст →
report_template::ConvertToSourceID(str, m_mapTextParam) [ДОБАВИТЬ в report_template — тело
не разобрано, для off-device можно вернуть str без ремапа]; tokens=KMeaStringUtil::SplitStr(
str, ","); key=tokens.back(); path=MAP_LOCAL_PIC_PFINFO[key]; valid=!QImage(path).isNull();
возврат path ВСЕГДА (даже если невалиден). bool& = валидность файла (QImage загрузился),
НЕ путь. НЕ ОПРЕДЕЛЕНО (помечено): MAP_LOCAL_PIC_PFINFO (глоб. .bss-кэш source-id→путь,
рантайм; ctor сидит demo-путь GetReadOnlyBaseDir()+REPORT_TEMPLATE_DEMO_IMG_RO) — для
self-test замокать через SetTextParam/прямую карту; ConvertToSourceID тело.

**ПОСЛЕДНЕЕ (эта сессия): `KTextBlock`** (`app/src/report/`, self-test `textblock`) — модель
