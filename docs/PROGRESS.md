...
СЛЕДУЮЩЕЕ ПО UI: портировать базовый `KDialog` (разблокирует хром всех диалогов), затем
остальные диалоги. Существующие UI-классы строят layout ВРУЧНУЮ (без Ui_-файлов), стили —
через `theme::` (qss/black/style.qss прошивки).
