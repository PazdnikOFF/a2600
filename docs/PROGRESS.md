...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 45  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** реализовано ~67/491 класса, **45 self-test-режимов**
(все PASS, регрессия прогнана — `tools/selftest.sh`), off-device-ядро ROADMAP Фаз A/B/C/D
в основном закрыто. KVideoProxy 57/116.
**ПОСЛЕДНЕЕ (эта сессия): INI-движок ядра `KConfig` + два фасада над ним.**
...
(1) **KTemplateCfg (26)** — кэш конфигов шаблонов отчётов (ReportTemplateConfig.xml/
TemplateTypes.xml/SubContentList.xml); частично тянет KTemplateEditDocument (UI-рендер) —
брать только off-device-часть;
