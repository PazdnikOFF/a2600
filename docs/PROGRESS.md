...
⭐⭐⭐ CLUSTER 3 (TABLE-MVC) ИСЧЕРПАН: 4 триплета (KExamImg/KUnusedImg/KExamDetail/KDICOMSrvList). Итого 3 портируемых UI-кластера (annotation-editor/pinyin-IME/table-MVC) — KDICOMSrvList закрывает последний. Остаток: Cluster 2 хвост (InputMethod-роутинг + KGooglePinyin-движок). COVERAGE.md регенерён: 181 кл/1842 мет (26.3%), было 12.8%. + полный экран patient-management (все 3 страницы patient/exam/dicom с реальными тулбарами) собран из библиотеки виджетов. — все кастом-виджеты, что подставлялись stock-Qt заглушками в ~60 диалогах, теперь имеют бинарно-точные реализации. Все скриншот-сверены, регрессия 106/106 всю сессию.
