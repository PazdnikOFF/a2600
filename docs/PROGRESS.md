...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 77  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **77 self-test-режимов** (все PASS, регрессия —
...
затронуто 85 классов / 909 методов (14.1%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KSysPrintData` (15 методов) — персист конфигурации принтеров
