...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 43  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** реализовано ~64/491 класса, **43 self-test-режима**
(все PASS, регрессия прогнана — `tools/selftest.sh`), off-device-ядро ROADMAP Фаз A/B/C/D
в основном закрыто. KVideoProxy 57/116.
**ПОСЛЕДНЕЕ (эта сессия): конфиг DICOM-сервисов `link-dicom.json`** — 8 классов
...
СЛЕДУЮЩИЕ КАНДИДАТЫ (аудит бинарника, отобраны как off-device+самодостаточные):
(1) **KPatientListConfigSetupHandler (28) + KWorkListConfigSetupHandler (25)** — точная
копия паттерна готового KExamListConfigHandler (`examcfg`), файлы `patientsetup.ini`/
`worklistsetup.ini`, ~53 метода за итерацию — дешёвое добивание;
(2) **KConfig (40)** — kernel-движок INI (LoadConfigFile/EraseComment/TrimBlankSpace/
Read*/WriteData/Save), даёт 1:1-семантику дефолтов/комментариев вместо QSettings;
(3) **KTemplateCfg (26)** — кэш конфигов шаблонов отчётов (частично тянет UI-рендер);
(4) **KControlINI (24)** — control.ini/лицензия, РИСК: шифрованный вход
(CipherFile2PlainFile+md5sum), формат придётся реверсить — может съесть итерацию.
