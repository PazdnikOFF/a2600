...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 76  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **76 self-test-режимов** (все PASS, регрессия —
...
затронуто 84 класса / 894 метода (13.9%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KColdLightConfig` — параметры авто-диммирования
...
B. ✅ РАЗВЕДКА ПРОВЕДЕНА (субагент, эта сессия). Результат — очередь непрочитанных конфигов
   ...
   B2. `system/printer/KSysPrintService.xml` + `KSyPrintUrlDriver.xml` — читатель НАЙДЕН:
   ...
   B3. `system/autotest/*/{autotest.xml,keyboard.ini,reset.ini,scope.ini,autotest-OSD.ini}` —
   ...
   B6. `presetdata/thirdpartylegalnotice/thirdPartyLegalNotices.txt` — читатель
