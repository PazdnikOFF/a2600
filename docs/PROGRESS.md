...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 68  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **68 self-test-режимов** (все PASS, регрессия —
...
затронуто 77 классов / 771 метод (12.0%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KObject`/`KMessage`/`KPublishManager` — in-process шина сообщений
