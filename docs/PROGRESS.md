...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 67  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **67 self-test-режимов** (все PASS, регрессия —
...
затронуто 74 класса / 752 метода (11.7%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `PatientExamData` (8 статических методов, self-test `examdata`)** —
