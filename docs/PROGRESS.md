...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 63  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **63 self-test-режима** (все PASS, регрессия —
...
затронуто 71 класса / 721 метода (11.2%)**. Это нижняя оценка (считает совпадение имён;
...
**ПОСЛЕДНЕЕ (эта сессия): `KImageBlock`** (`app/src/report/`, self-test `imageblock`) — модель
