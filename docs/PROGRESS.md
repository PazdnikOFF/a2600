...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** реализовано ~56/491 класса, **39 self-test-режимов**
...
(`encstyle`), KExamListRecordFileUpdate::GetFiletypeNumFromPath (`recfiles`).
...
**ПРОДОЛЖИТЬ (CORE off-device):** чистые мелкие config-классы истощаются — следующая разведка
субагентом должна искать НЕ тронутые конфиг-файлы update/root + проверять, что метод не тянет
device/неизвестную схему. Либо крупный заход KVideoProxy VideoParamChanged (нужно решение
пользователя по инфраструктуре сигналов). Приём: `comm -23 <методы-бинарника> <наши>`.
