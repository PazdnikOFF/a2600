...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 73  FAIL: 0"
```
...
затронуто 81 класс / 803 метода (12.5%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KControlProc` — крипто-ядро машинного контроля (self-test
...
ОСТАЛОСЬ у KControlProc (~25 методов, часть device/ini): Start/StopEndoMc, IsEndoMatch,
CheckLicense, GetMcFilenameList, ImportMatchLicense, IsMd5sumUsed, GetDelayTime и др.
