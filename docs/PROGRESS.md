...
```bash
tools/selftest.sh "$SCR/uibuild/ui_preview"     # → "PASS: 75  FAIL: 0"
```
...
**ТЕКУЩАЯ ПОЗИЦИЯ (обновлять!):** **75 self-test-режимов** (все PASS, регрессия —
...
затронуто 84 класса / 888 методов (13.8%)** (report_template 28/36 = 78% — свободные функции
...
**ПОСЛЕДНЕЕ (эта сессия): `KEncStyle` — video.ini-геттеры per-скоп (расширен self-test
...
GetEndoDisplayModel — off-device, но нужен KAccount::CurrentRole()<=1 && стиль=="PyCkeun"
(таблица из 5: EG-X20→G, EC-X20→C, EC-X20L→C, EB-X20→B, EB-X20T→B) — отложено.
...
A. ✅ СДЕЛАНО (см. «ПОСЛЕДНЕЕ» выше): KEncStyle остаток off-device — все 13 video.ini-геттеров
   ...
   device (GetEndoModel + no-arg twins + getIsDefaultMatch) и отложенный GetEndoDisplayModel.
