#include "KPatientListViewUi.h"
#include "KTableView.h"
#include "KPagePushButton.h"
#include "KPageLineEdit.h"
#include "KPatientListSearch.h"
#include "sys/KEnvConfig.h"

#include <QGroupBox>
#include <QLabel>
#include <QVBoxLayout>
#include <QLineEdit>
#include <QDir>

KPatientListViewUi::KPatientListViewUi(QWidget *parent)
    : QWidget(parent)
{
    // Реф. ctor @0x7e5650: setupUi → embed search → InitTable → InitPageButton →
    // InitLineEditPage → RefreshPageInfo → SubscribeMsg(device) → Initconnect.
    setupUi();

    // Встроенный поиск — дитя widget_search (реф. new KPatientListSearch(widget_search)).
    m_search = new KPatientListSearch(m_widgetSearch);
    QVBoxLayout *sv = new QVBoxLayout(m_widgetSearch);
    sv->setContentsMargins(0, 0, 0, 0);
    sv->addWidget(m_search);

    InitTable();
    InitPageButton();
    InitLineEditPage();
    RefreshPageInfo();

    // Реф. Initconnect (частично; device eventFilter-навигация → в порте штатные clicked).
    connect(m_btnHome, &KPagePushButton::clicked, this, &KPatientListViewUi::JumpToHeadPage);
    connect(m_btnPre, &KPagePushButton::clicked, this, &KPatientListViewUi::ClickPatientListBtnPre);
    connect(m_btnNext, &KPagePushButton::clicked, this, &KPatientListViewUi::ClickPatientListBtnNext);
    connect(m_btnTail, &KPagePushButton::clicked, this, &KPatientListViewUi::JumpToTailPage);
    connect(m_editPage, &QLineEdit::returnPressed, this, &KPatientListViewUi::JumpToCustomPage);
    // Поиск → запрос (реф. QueryItems → OnQuery).
    // (KPatientListSearch порт эмитит свой QueryItems; здесь не подписываемся жёстко, т.к.
    //  сигнатура зависит от порта поиска — оставлено как seam.)
}

void KPatientListViewUi::setupUi()
{
    setObjectName(QStringLiteral("KPatientListViewUi"));
    resize(1630, 1024);
    setMinimumSize(1630, 1024);
    setMaximumSize(1630, 1024);

    QVBoxLayout *v2 = new QVBoxLayout(this);
    v2->setObjectName(QStringLiteral("verticalLayout_2"));
    v2->setContentsMargins(0, 0, 0, 0);
    v2->setSpacing(0);

    m_grpView = new QGroupBox(this);
    m_grpView->setObjectName(QStringLiteral("grp_view"));
    m_grpView->setTitle(QString());
    v2->addWidget(m_grpView);

    QVBoxLayout *v = new QVBoxLayout(m_grpView);
    v->setObjectName(QStringLiteral("verticalLayout"));
    v->setContentsMargins(18, 4, 0, 0);

    m_widgetSearch = new QWidget(m_grpView);
    m_widgetSearch->setObjectName(QStringLiteral("widget_search"));
    m_widgetSearch->setMinimumWidth(1591);
    v->addWidget(m_widgetSearch);

    m_tableView = new KTableView(m_grpView);
    m_tableView->setObjectName(QStringLiteral("tableView"));
    m_tableView->setFixedWidth(1591);
    v->addWidget(m_tableView);

    // Пейджер-бар — абсолютная геометрия (реф. без layout).
    m_pagerBar = new QWidget(m_grpView);
    m_pagerBar->setObjectName(QStringLiteral("widget"));
    m_pagerBar->setMinimumHeight(70);
    v->addWidget(m_pagerBar);

    m_labelDbRecord = new QLabel(m_pagerBar);
    m_labelDbRecord->setObjectName(QStringLiteral("label_db_record"));
    m_labelDbRecord->setGeometry(20, 1, 158, 64);

    m_labelHint = new QLabel(m_pagerBar);
    m_labelHint->setObjectName(QStringLiteral("label_hint"));
    m_labelHint->setGeometry(182, 16, 585, 50);
    m_labelHint->setAlignment(Qt::AlignTop | Qt::AlignLeft);
    m_labelHint->setText(tr("TR_DCOPETExam") + QStringLiteral("\r\n\r\n")
                         + QStringLiteral("F4/F5: ") + tr("TR_SCAPage"));

    m_btnHome = new KPagePushButton(m_pagerBar);
    m_btnHome->setObjectName(QStringLiteral("btn_home"));
    m_btnHome->setGeometry(767, 10, 52, 34);
    m_btnPre = new KPagePushButton(m_pagerBar);
    m_btnPre->setObjectName(QStringLiteral("btn_pre"));
    m_btnPre->setGeometry(835, 10, 52, 34);

    m_editPage = new KPageLineEdit(m_pagerBar);
    m_editPage->setObjectName(QStringLiteral("edit_page"));
    m_editPage->setGeometry(911, 10, 52, 32);
    m_editPage->setAlignment(Qt::AlignRight | Qt::AlignVCenter);

    m_labelPage = new QLabel(m_pagerBar);
    m_labelPage->setObjectName(QStringLiteral("label_page"));
    m_labelPage->setGeometry(980, 14, 41, 26);

    m_btnNext = new KPagePushButton(m_pagerBar);
    m_btnNext->setObjectName(QStringLiteral("btn_next"));
    m_btnNext->setGeometry(1021, 10, 52, 34);
    m_btnTail = new KPagePushButton(m_pagerBar);
    m_btnTail->setObjectName(QStringLiteral("btn_tail"));
    m_btnTail->setGeometry(1090, 10, 52, 34);
}

void KPatientListViewUi::InitTable()
{
    // Реф. InitTable: заголовки через tr(), col0 — чекбокс. Набор колонок конфиг-управляемый
    // (в порте — фиксированный видимый набор).
    QVector<KHeaderProperty> h = {
        {QString(), QStringLiteral("sel"), 40, true},
        {tr("TR_PID"), QStringLiteral("PID"), 130, true},
        {tr("TR_Nme"), QStringLiteral("Name"), 180, true},
        {tr("TR_Gdr"), QStringLiteral("Gender"), 90, true},
        {tr("TR_Age"), QStringLiteral("Age"), 80, true},
        {tr("TR_EItem"), QStringLiteral("ExamItem"), 160, true},
        {tr("TR_Aplct"), QStringLiteral("Application"), 140, true},
        {tr("TR_PDate"), QStringLiteral("PatientDate"), 150, true},
        {tr("TR_ADate"), QStringLiteral("AcqDate"), 150, true},
        {tr("TR_Stts"), QStringLiteral("Status"), 120, true}};
    m_tableView->InitTableView(h, QStringLiteral("PID"), 15, 0);
    // Реф.: страница подгружается по SigGetDataFromDB.
    connect(m_tableView, &KTableView::SigGetDataFromDB,
            this, &KPatientListViewUi::OnGetPatientListDataFromDB);
}

void KPatientListViewUi::InitPageButton()
{
    // Реф. InitButton(QMap normal/hover/disable) из ProjectPresetPath + "patient/pageturning/".
    const QString base = QDir(QString::fromStdString(KEnvConfig::GetInstance().GetReadOnlyBaseDir()))
                             .absoluteFilePath(QStringLiteral("patient/pageturning/"));
    auto icons = [&](const QString &type) {
        QMap<QString, QString> m;
        m.insert(QStringLiteral("normalIcon"), base + "page_" + type + "_normal.png");
        m.insert(QStringLiteral("hoverIcon"), base + "page_" + type + "_hover.png");
        m.insert(QStringLiteral("disableIcon"), base + "page_" + type + "_disable.png");
        return m;
    };
    m_btnHome->InitButton(icons(QStringLiteral("head")));
    m_btnPre->InitButton(icons(QStringLiteral("front")));
    m_btnNext->InitButton(icons(QStringLiteral("next")));
    m_btnTail->InitButton(icons(QStringLiteral("tail")));
}

void KPatientListViewUi::InitLineEditPage()
{
    m_editPage->SetMaximumValue(m_maxPage);
    m_editPage->setText(QString::number(m_currentPage));
}

void KPatientListViewUi::RefreshPageInfo()
{
    // Реф. RefreshPageInfo: метки count/maxPage/curPage + enable/disable кнопок.
    m_labelDbRecord->setText(tr("TR_Ttl:") + QString::number(m_count));
    m_labelPage->setText(QString::number(m_maxPage));
    m_editPage->SetMaximumValue(m_maxPage);
    m_editPage->setText(QString::number(m_currentPage));

    const bool single = (m_maxPage <= 1);
    m_btnHome->setEnabled(!single && m_currentPage > 1);
    m_btnPre->setEnabled(!single && m_currentPage > 1);
    m_btnNext->setEnabled(!single && m_currentPage < m_maxPage);
    m_btnTail->setEnabled(!single && m_currentPage < m_maxPage);
}

void KPatientListViewUi::gotoPage(int page)
{
    if (page < 1) page = 1;
    if (page > m_maxPage) page = m_maxPage;
    m_currentPage = page;
    OnGetPatientListDataFromDB(page - 1);   // реф.: 0-based страница в модель
    RefreshPageInfo();
}

void KPatientListViewUi::OnGetPatientListDataFromDB(int page)
{
    // Реф.: KPatientListDBTableHandler::GetPageRecordFromDb → SetModelData. DEVICE-STUB: провайдер.
    if (m_pageProvider && m_tableView->GetModel())
        m_tableView->GetModel()->SetModelData(page, m_pageProvider(page));
}

void KPatientListViewUi::JumpToHeadPage() { gotoPage(1); }
void KPatientListViewUi::ClickPatientListBtnPre() { gotoPage(m_currentPage - 1); }
void KPatientListViewUi::ClickPatientListBtnNext() { gotoPage(m_currentPage + 1); }
void KPatientListViewUi::JumpToTailPage() { gotoPage(m_maxPage); }

void KPatientListViewUi::JumpToCustomPage()
{
    // Реф.: валидировать 1≤n≤maxPage из edit_page.
    bool ok = false;
    const int n = m_editPage->text().trimmed().toInt(&ok);
    if (ok)
        gotoPage(n);
}

void KPatientListViewUi::OnQuery(const QMap<QString, QString> &)
{
    // Реф. OnQuery: слить условия, перезапросить page 0. DEVICE-STUB.
    gotoPage(1);
}

void KPatientListViewUi::SetPageProvider(
    std::function<QVector<QMap<QString, QString>>(int)> fn, int totalPages, int totalCount)
{
    m_pageProvider = std::move(fn);
    m_maxPage = totalPages > 0 ? totalPages : 1;
    m_count = totalCount;
    gotoPage(1);
}
