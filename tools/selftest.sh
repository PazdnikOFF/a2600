...
remoteswitch dcmfmt fxpt language unicodetext encstyle dsreal meaxml templatecfg templatelib encset imageblock reporttmplmgr legalnotice factoryopt docsync initdocreal"
# Режимы без ENDO_ROOT (чистая логика / своя БД).
PLAIN_MODES="cornercut quickinput examcfg exam filebackup update reportdb
camera endoscope dimming smalllang videoplayer reportedit imgproc exportrec savefile dbservice dispparam pattime recfiles kconfig strutil templateparam dbfileop patstr stopwatch patient doctor dbstr session textblock tableblock titletableblock reporttmpl examdata kobject xmlparser dbsqlite des templetmodel docgen monitor rtcreator videoipc screenmng rtdatasource listbuff quickinputdb quickinputmodel reporttitledb viewbase videolabel rtsimple rtcreators rttext rtimage rttable findcell selectitem docedit movefb changecolor initdoc"
